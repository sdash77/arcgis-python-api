...
def export_notebooks(
    ...
    replace_img_path=False,
    img_prefix=DEFAULT_IMG_PREFIX,
    replace_video_path=False,
    ...
    """
    log_func(
        f"Converting notebooks at {root_path} to html, placing output "
    ...
    if True in list(map(lambda arg: arg in root_path, unsafe_dirs)):
        log_func(
            "ATTEMPTING TO CONVERT NOTEBOOKS IN DESIGNATED 'unsafe_dirs'"
        ...
        for curr_file in fnmatch.filter(file_list, "*.ipynb"):
            ...
            if replace_img_path:
                for img in soup.findAll("img"):
                    if ("http" not in img["src"]) and ("data:" not in img["src"]):
                        ...
                        img["src"] = new_img_src
                        log_func(
                            "IMG replaced {} with {}".format(old_img_src, new_img_src)
                        )
            ...
            if replace_video_path:
                for vid in soup.findAll("video"):
                    if ("http" not in vid["src"]) and ("data:" not in vid["src"]):
                        # Will match with all relative paths in the notebook
                        old_vid_src = vid["src"]
                        ...
                        vid["src"] = new_vid_src
                        log_func(
                            "video replaced {} with {}".format(old_vid_src, new_vid_src)
                        )
            ...
            if replace_full_urls:
                for link in soup.findAll("a", href=True):
                    if "developers.arcgis.com" in link["href"]:
                        ...
                        link["href"] = rel_url
                        log_func("HREF Replaced {} with {}".format(full_url, rel_url))
...
def _assemble_button_html(directory, curr_file, href_url):
    ...
    button_html = (
        ...
        '#Download-and-run-the-sample-notebooks">' + "Download the samples</a>"
    )
    button_html = (
        button_html + " " + f'<a class="btn" href="{href_url}"'
