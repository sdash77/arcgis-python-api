...
def stage_notebooks_for_dev_web_repo(
    notebooks_root_dir,
...
def _convert_notebooks_to_html(notebooks_root_dir, output_dir, log_func):
    ...
    guide_html_dir = os.path.join(output_dir, "src", "python", "guide")
    samples_html_dir = os.path.join(output_dir, "src", "python", "sample-notebooks")
...
def _copy_imgs(notebooks_root_dir, output_dir):
    ...
    img_html_dir = os.path.join(output_dir, "src", "assets", "img", "python-graphics")
    if os.path.isdir(img_html_dir):
        recursive_file_copy(src_dir_root=img_src_dir, dst_dir_root=img_html_dir)
    else:
...
def _copy_videos(notebooks_root_dir, output_dir):
    ...
    video_html_dir = os.path.join(output_dir, "static", "python", "assets", "video")
    if os.path.isdir(video_html_dir):
        recursive_file_copy(src_dir_root=video_src_dir, dst_dir_root=video_html_dir)
    else:
