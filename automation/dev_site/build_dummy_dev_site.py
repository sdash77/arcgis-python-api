...
from automation.dev_site.stage_notebooks_for_dev_web_repo import recursive_file_copy
...
def build_dummy_dev_site(notebooks_root_dir, **kwargs):
    ...
    # Copy all images
    recursive_file_copy(src_dir_root=img_dir, dst_dir_root=output_dir)
...
def _write_index_html_file_for_outputted_html(output_dir):
    with open(os.path.join(output_dir, "index.html"), "w") as f:
        ...
        f.write(
            "<p>Press any of the below links to view a dummy "
        ...
        for html_file in glob.glob(os.path.join(output_dir, "*.html")):
            html_file_name_no_ext = os.path.splitext(os.path.basename(html_file))[0]
            f.write(
                '<p><a href="./{0}.html">{0}</a></p>\n'.format(html_file_name_no_ext)
            )
