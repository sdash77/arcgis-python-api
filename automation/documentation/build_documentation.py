...
DOCS_DIR = os.path.abspath(os.path.join(GEOSAURUS_ROOT_DIR, "docs", "api_ref"))
infile = os.path.abspath(os.path.join(STAGING_DIR, "coverage", "undoc.pickle"))
outfile = os.path.abspath(os.path.join(STAGING_DIR, "undocumented.txt"))
...
def build_documentation(*args, **kwargs):
    ...
    log.info("Documentation building finished! Any results in {}".format(STAGING_DIR))
...
def _build_doc_for_unix():
    ...
    final_bash_command = "cd {} && {} {}".format(
        DOCS_DIR, make_command, flag_to_force_output_to_staging_dir
    )
...
def _build_doc_for_windows():
    ...
    final_make_command = 'cd "{}" && {} {}'.format(
        DOCS_DIR, bat_make_command, flag_to_force_output_to_staging_dir
    )
