...
def run_notebook_tests(
    ...
    **kwargs,
    ...
    suite = _discover_tests_get_suite(
        notebooks_root_dir=notebooks_root_dir,
...
def _discover_tests_get_suite(
        ...
        for name in [file_ for file_ in files if ".ipynb" in file_]:
            ...
            if (".ipynb_checkpoints" not in notebook_path) and (
                ...
                test = TestNotebook(
                    ...
                    cell_timeout_sec=cell_timeout_sec,
                    jenkins_job_url="/".join(
                        [jenkins_root, "job", automation_type, str(build_number), ""]
                    ),
    ...
    if output_suite.countTestCases() == 0:
        raise RuntimeError(
            "0 Notebooks found to run: Make sure '{}' "
            "contains runnable notebooks.".format(notebooks_root_dir)
        )
    else:
        log.info("Found {} notebooks to run".format(output_suite.countTestCases()))
    ...
    with open(os.path.join(STAGING_DIR, "index.html"), "w") as f:
        ...
        for notebook_html_file in glob(os.path.join(STAGING_DIR, "*.html")):
            notebook_html_file_name_no_ext = os.path.splitext(
                os.path.basename(notebook_html_file)
            )[0]
            f.write(
                '<p><a href="./{0}.html">{0}</a></p>\n'.format(
