...
try:
    ...
    print(
        "ERROR: the arcgis package being tested should be {}".format(expected_file_path)
    )
...
def remove_tests_to_skip(tests, test_suite_names_to_skip):
    ...
    assert isinstance(test_suite_names_to_skip, list), type(test_suite_names_to_skip)
...
test_results_dir = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "test-results"
)
...
def run_test_cases(args):
    ...
    try:
        ...
        print(
            "Unhandled exception on setting up unit testing. Still attemping to run.."
        )
...
def _setup_testing(args):
    ...
    parser.add_argument("--skip", dest="skip", help="Name of tests to skip")
    parser.add_argument(
        "names",
        metavar="N",
        nargs="+",
        help="Name of file containing test cases, or name of directory containing "
    ...
    for name in names:
        if os.path.isdir(name):
            ...
            suites = suites + [unittest.defaultTestLoader.loadTestsFromName(name)]
...
if __name__ == "__main__":
    print(
        "You should not be running tests by calling this file. Use the "
