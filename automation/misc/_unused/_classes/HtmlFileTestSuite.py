    ...
    def _discover_img_tests(self):
        ...
        for img in self.parser.imgs:
            if "data:" in img.url:
                ...
                self.suite.addTest(
                    self._PassTestCaseClass(
                        "Found embedded "
                        "'data:' <img> tag in file {filename} at line {line}, "
                        "char {char}. Passing.".format(
                            filename=filename,
            ...
            self.suite.addTest(self._TestHtmlMatchClass(img, check_https=True))
    ...
    def _discover_href_tests(self):
        ...
        for href in self.parser.hrefs:
            if href.url.startswith("#"):
                self.suite.addTest(
                    self._PassTestCaseClass(
                        "Found " "fragment identifier {}. Auto-passing".format(href.url)
                    )
                )
                continue
            if href.url.startswith("/"):
            ...
            self.suite.addTest(self._TestHtmlMatchClass(href, check_https=False))
