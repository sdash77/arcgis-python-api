...
class TestHtmlMatch(unittest.TestCase, RenameableClass):
    ...
    def _check_https(self, request):
        if (
            (self.check_https)
            and ("http:" in self.html_match.url)
            and (self.base_server not in self.html_match.url)
        ):
            # All external links not hosted by base_server should be https
            self.skipTest(
                "url needs to be https! See https://bit.ly/2GYOkes."
    ...
    def _get_html_match_info_str(self):
        ...
        return (
            "File {filename} referenced url '{url}' at line {line}, char "
            "{char}".format(
                filename=filename,
