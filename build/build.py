...
BASE_BUILD_CMD = (
    ...
    + f"-c {ESRI_CHANNEL_DEV} -c {ESRI_REQUESTS_CHANNEL} -c esri "
    + "arcgis --py {python_version} --output-folder {output_dir}"
)
BASE_CONVERT_CMD = (
    "conda convert -f -p {os_build_target} {conda_package} " "-o {output_dir}"
)
...
BUILD_DIR = os.path.abspath(os.path.join(GEOSAURUS_ROOT_DIR, "build"))
BUILD_OUTPUT_DIR = os.path.abspath(os.path.join(BUILD_DIR, "output"))
BUILD_OUTPUT_PIP_DIR = os.path.abspath(os.path.join(BUILD_OUTPUT_DIR, "pip"))
SRC_DIR = os.path.abspath(os.path.join(GEOSAURUS_ROOT_DIR, "src"))
SRC_DIST_DIR = os.path.abspath(os.path.join(SRC_DIR, "dist"))
META_YAML_FILE_PATH = os.path.abspath(os.path.join(BUILD_DIR, "arcgis", "meta.yaml"))
...
def _parse_cmd_line_args():
    parser = argparse.ArgumentParser(
        description="Builds arcgis conda "
...
def _no_target_specified(args):
    ...
    return (not args.all) and (not args.os) and (not args.python) and (not args.pip)
...
def build_conda_packages_default(clear_output_folder=True):
    ...
    for python_version in DEFAULT_PYS:
        _run_conda_build_command(
            python_version=python_version, output_dir=BUILD_OUTPUT_DIR
        )
...
def build_conda_packages(
    os_build_targets, python_versions, build_number=0, clear_output_folder=True
):
    ...
    if clear_output_folder:
        _clear_output_folder()
    _check_edge_cases(os_build_targets, python_versions)

    for python_version in python_versions:
        with _empty_tmp_dir() as tmp_dir:
            _run_conda_build_command(python_version=python_version, output_dir=tmp_dir)
            _convert_conda_package(
                conda_package=_find_conda_package(tmp_dir),
                os_build_targets=os_build_targets,
...
def upload_any_conda_packages_in_output_folder():
    ...
    """
    log.info(
        "Uploading any conda packages in {}. Make sure anaconda login "
...
def _is_unix(os_build_target):
    return (os_build_target in SUPPORTED_LINUX) or (os_build_target in SUPPORTED_OSX)
...
def _clear_output_folder():
    ...
    for filtered_item in [
        os.path.join(BUILD_OUTPUT_DIR, dir_item)
...
def _check_edge_cases(os_build_targets, python_versions):
    ...
    for os_build_target in os_build_targets:
        if _is_windows(os_build_target) and _is_current_os_unix():
            ...
            raise RuntimeError(
                "You're building for {}: Windows pkgs cannot "
...
def _run_conda_build_command(python_version, output_dir):
    _run_shell_cmd(
        BASE_BUILD_CMD.format(
            build_dir=BUILD_DIR, python_version=python_version, output_dir=output_dir
        )
    )
...
def _convert_conda_package(conda_package, os_build_targets, output_dir):
    for os_build_target in os_build_targets:
        _run_conda_convert_command(
            conda_package=conda_package,
            os_build_target=os_build_target,
            output_dir=output_dir,
        )
        _run_conda_index_command(output_dir=output_dir, os_build_target=os_build_target)


def _run_conda_convert_command(conda_package, os_build_target, output_dir):
    output = _run_shell_cmd(
        BASE_CONVERT_CMD.format(
            conda_package=conda_package,
            os_build_target=os_build_target,
            output_dir=output_dir,
        )
    ...
    if "Skipping conversion" in output:
        ...
        log.debug(
            "conda convert didn't complete: manually copied {} to "
...
def _run_conda_index_command(output_dir, os_build_target):
    ...
    """
    _run_shell_cmd(
        BASE_INDEX_CMD.format(output_dir=output_dir, os_build_target=os_build_target)
    )
...
def _run_shell_cmd(cmd):
    ...
    log.info(
        "Currently running cmd '{}'. After it completes, output will "
...
class _empty_tmp_dir:
    ...
    def __enter__(self):
        self.temp_folder = os.path.join(tempfile.gettempdir(), ".{}".format(uuid4()))
...
def _determine_current_os():
    ...
    return "{target_os}-{architecture}".format(
        target_os=target_os, architecture=architecture
    )
...
if __name__ == "__main__":
    try:
        _main()
        log.info(
            "Program successfully completed! Any generated packages in "
