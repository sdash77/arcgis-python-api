...
if HAS_ARCPY:
    class RobustReProjectionTest(unittest.TestCase):
        ...
        def test_create_from_wkid_arcgis(self):
            sdf2 = SpatialDataFrame(self._df, geometry=self._geoms, sr=_types.SpatialReference({'wkid' : 4326}))
            self.assertIsInstance(sdf2.sr.as_arcpy, arcpy.SpatialReference)
        def test_create_sdf_from_wkt_arcgis(self):
            sdf3 = SpatialDataFrame(self._df, geometry=self._geoms, sr=_types.SpatialReference({'wkt' : self._wkt}))
