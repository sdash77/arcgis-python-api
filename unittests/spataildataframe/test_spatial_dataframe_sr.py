...
geom_geom_arcgis_objs = [
    arcgis.geometry.Geometry(json.loads(arcpy.PointGeometry(arcpy.Point(X=1,Y=2), 4326).JSON)),
    arcgis.geometry.Geometry(json.loads(arcpy.PointGeometry(arcpy.Point(X=2,Y=3), 4326).JSON)),
    arcgis.geometry.Geometry(json.loads(arcpy.PointGeometry(arcpy.Point(X=4,Y=5), 4326).JSON))
]

ALL_GEOMS = [geom_geom_arcgis_objs,
             geom_geom_pt,
             geom_pts]
DF = pd.DataFrame([[1,2,3], [2,3,4], [3,4,5]], columns=['A', 'B', 'C'])
DF_GEOMS = pd.DataFrame([[1,2,3, geom_geom_arcgis_objs[0]],
