...
class Test_ResourceManager_portal(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.qalab_cls_path = cls.qalab_base_path + _conf_reader2['test_data']['qalab_ResourceManager_cls']
    ...
    def test_add_resources_single_file_root(self):
        ...
        try:
            ...
            self.assertTrue(add_result['success'], "Failed to add resource file at root level")
    ...
    def test_add_resources_multi_files_folders(self):
        ...
        try:
            ...
            full_file_list = [os.path.join(root, file)
                              for root, dir, file_list in os.walk(folder_to_add)
                              for file in file_list]
            print("Number of resources to be added in this test: " + str(len(full_file_list)))
            ...
            for now_file in full_file_list:
                ...
                prefix_safe = prefix.replace('\\', '/').replace(' ', '_')  # make URL safe
                ...
                self.assertTrue(add_result['success'], "Failed to add resource file: " + filename)
    ...
    def test_add_resources_archive(self):
        ...
        try:
            # add a json resource file
            file_to_add = os.path.join(self.qalab_cls_path, 'set2_vtpk_style_canvasdark.zip')
            ...
            print("Number of resource files from archive upload: " + str(len(resource_list)))
            self.assertGreaterEqual(len(resource_list), 1, "Number of resources did not increment for archive upload")
    ...
    def test_list_resources_small_anonymous_ago(self):
        ...
        vtl_item = _gis.content.get('92c551c9f07b4147846aae273e822714') #World street map night

        if vtl_item is None:
            print("Could not find Esri's vector tile service item: 92c551c9f07b4147846aae273e822714" +
                  "Skipping test case.")
        ...
        try:
            ...
            self.assertIsNotNone(vtl_resources, "Got a none object when listing an item with resources")
            print("Number of resource files: " + str(len(vtl_resources)))

            self.assertGreaterEqual(len(vtl_resources), 2, "Number of resource files less than usual")
    ...
    def test_list_resources_huge_anonymous_ago(self):
        ...
        if vtl_item is None:
            print("Could not find Esri's vector tile service item: 2e063e709e3446459f8538ed6743f879" +
                  "Skipping test case.")
            raise unittest.SkipTest
        ...
        try:
            ...
            self.assertGreaterEqual(len(vtl_resources), 10000, "Number of resource files less than usual")
    ...
    def test_get_resource_json_file_public_item(self):
        ...
        try:
            ...
            self.assertIsInstance(root_json, dict, "Got a non dict object for JSON resource file")
    ...
    def test_get_resource_bin_file_custom_download(self):
        ...
        self.assertTrue(add_result2['success'] and add_result['success'], "Failed to add resource files")

        # Download both json and png files to disk overriding defaults
        try:
            #create a timestamped folder to download this file
            output_folder = os.path.join(self.qalab_output_root + self.qalab_cls_name,
                                         self._testMethodName, self.time_stamp)
            os.makedirs(output_folder, exist_ok=True) #create intermediate dirs as necessary. If it exists use it.

            fld_png = res_mgr.get('fld/set2_fld_bin_res_file.png', out_folder=output_folder,
                                  out_file_name='a_sequoias_seed_is_tiny.png')
            fld_json = res_mgr.get('fld/root_resource_file.json', out_folder=output_folder,
            ...
            self.assertTrue('a_sequoias_seed_is_tiny.png' in fld_png,
                              'png file does not download with custom name')
            self.assertTrue('geosaurus' in fld_png, 'png file does not download to custom dir')

            self.assertTrue('as_its_a_type_of_pine.json' in fld_json,
    ...
    def test_update_resource_json_file(self):
        ...
        # add json file
        file_to_add2 = os.path.join(self.qalab_cls_path, 'root_resource_file.json')
        file_to_update = os.path.join(self.qalab_cls_path,'overwrite', 'root_resource_file.json')
        add_result2 = res_mgr.add(file_to_add2, folder_name='fld')
        ...
        try:
            ...
            self.assertIsInstance(original_json, dict, "Did not get dict when getting json resource file")
            self.assertTrue(original_json['text'] == 'hello arcgis', 'content of original JSON file is wrong')
            ...
            self.assertIsInstance(updated_json, dict, "Did not get dict when getting json resource file")
            self.assertTrue(updated_json['text'] == 'hello Python folks', 'contents of updated JSON file is wrong')
    ...
    def test_remove_resource_one_file(self):
        ...
        try:
            ...
            self.assertEqual(len(original_list),2, "Number of original resources is not 2")
    ...
    def test_remove_resources_all_files(self):
        ...
        try:
            ...
            original_list = res_mgr.list()
            self.assertGreaterEqual(len(original_list), 5, "Number of original resources is not at least 5")
