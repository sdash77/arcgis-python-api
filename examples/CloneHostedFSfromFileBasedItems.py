...
def copy_item(target, owner, folder, item):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        metadata_file = item.download_metadata(temp_dir)  # If no metadata, comes back as None
        # Add the item to the target portal
        try:
            dup_item = target.content.add(new_item, data_file, thumbnail_file, metadata_file, owner, folder)
        except RuntimeError as e2:
            print(e2)
            return None
        return dup_item
            ...
            if relitem.type == 'Feature Service':
                ...
                if copied_item is not None:
                    ...
                    if success:
                        ...
                        print("\tSuccessfully transitioned {} to target Portal...\n".format(item.name))
