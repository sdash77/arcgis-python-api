...
AGOL_PASSWORD = None
VERIFY=False
TEST_PORTAL = "http://acpythondev.esri.com/portal"
TEST_SERVER = "http://acpythondev.esri.com/server"
TEST_ACCOUNT_PORTAL = "admin"
TEST_ACCOUNT_PORTAL_PW = "esri.agp"
if AGOL_USERNAME and AGOL_PASSWORD:
    ...
    class ServerAGOLTest(unittest.TestCase):
        ...
        def test_reports(self):
            res = []
            urls = self._gis._con.get(url="%s/portals/%s/urls" % (self._gis._portal.resturl,
        ...
        def test_get_not_found(self):
            from arcgis.features import FeatureLayerCollection
            res = []
            urls = self._gis._con.get(url="%s/portals/%s/urls" % (self._gis._portal.resturl, self._gis.properties.id), params={'f': 'json'})
...
class ServerPortalTest(unittest.TestCase):
    ...
    def setUp(self):
        self._gis = GIS(TEST_PORTAL, TEST_ACCOUNT_PORTAL, TEST_ACCOUNT_PORTAL_PW, verify_cert=VERIFY)
    def test_server_portal_not_gis(self):
        """tests creating a Server object"""
        from arcgis.gis.server import Server
        s = Server(url=TEST_SERVER, gis=None,
               username="admin", password="esri.agp",
               tokenurl="%s/admin/generateToken" % TEST_SERVER)
...
class ServerCatalogCreationTests(unittest.TestCase):
    ...
    def test_105_catalog_token_login(self):
        ...
        url_105 = URLS[5]
        server =  ServicesDirectory(url=url_105,
                        username=TEST_ACCOUNT_PORTAL,
                        password=TEST_ACCOUNT_PORTAL_PW,
                        tokenurl="%s/admin/generateToken" % TEST_SERVER)
        self.assertIsInstance(server,  ServicesDirectory)
...
############################################################################
#@unittest.SkipTest
class ServerAdminServicesTest(unittest.TestCase):
    def test_content(self):
        """catalog 10.5 content listing"""
        url_105 = URLS[5]
        gis = GIS(url=TEST_PORTAL,
                  username=TEST_ACCOUNT_PORTAL,
                  password=TEST_ACCOUNT_PORTAL_PW,
                  verify_cert=False)
        server =  ServicesDirectory(url=TEST_SERVER,
                        username=TEST_ACCOUNT_PORTAL,
                        password=TEST_ACCOUNT_PORTAL_PW,
                        portal_connection=gis).admin
