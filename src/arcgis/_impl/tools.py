...
class _RasterAnalysisTools(BaseAnalytics):
    ...
    def summarize_categorical_raster(
        ...
        **Argument**                             **Description**
        ...
        input_categorical_raster = self._layer_input(input_layer=input_categorical_raster)
        ...
        if folder is not None:
            ...
            output_summary_table_name = json.dumps({"serviceProperties": {"name": output_summary_table_name}})
    ...
    def train_random_trees_regression_model(
        ...
        **Argument**                             **Description**
        ...
        input_rasters = self._set_multiple_raster_inputs(
            input_rasters=input_rasters
        )

        if isinstance(input_target_data, _FEATURE_INPUTS):
            input_target_data = self._feature_input(
                input_target_data
            )
        elif isinstance(input_target_data, Item):
            input_target_data = {
                "itemId": input_target_data.itemid
            }
        else:
            input_target_data = self._layer_input(
                input_target_data
            )
        ...
        if output_sample_features_name is not None:
            output_sample_features_name = json.dumps({"serviceProperties": {"name": output_sample_features_name}})

        if output_importance_table_name is not None:
            output_importance_table_name = json.dumps({"serviceProperties": {"name": output_importance_table_name}})
