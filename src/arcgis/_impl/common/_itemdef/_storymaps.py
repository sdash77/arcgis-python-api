...
from arcgis._impl.common._clone import _search_org_for_existing_item, _share_item_with_groups
try:
...
class _StoryMapDefinition(CloneNode):
    ...
    """
    def __init__(self, target, info, data, sharing=None, thumbnail=None, portal_item=None, folder=None, item_extent=None, search_existing=True, owner=None, **kwargs):
    ...
    def clone(self):
        ...
        if not new_item:
            ...
            data = self.data
            web_maps = set([v['data']['itemId'] for k, v in data['resources'].items() \
                            if v['type'].lower().find('webmap')>-1])
            express_maps = set([v['data']['itemId'] for k, v in data['resources'].items() \
                                if v['type'].lower().find('expressmap')>-1])

            webmap_mapper = {}
            for wm in web_maps:
                webmap_to_copy = self.portal_item._gis.content.get(wm)
                cloned_webmaps = self.target.content.clone_items([webmap_to_copy], search_existing_items=False) # Clones the WebMap
                webmap_mapper[webmap_to_copy.id] = [i.id for i in cloned_webmaps if i.type == 'Web Map']
                if len(webmap_mapper[webmap_to_copy.id]) == 1:
                    webmap_mapper[webmap_to_copy.id] = webmap_mapper[webmap_to_copy.id][0]
            ...
            if self.resources:
                new_item.resources.add(self.resources,
                                       archive=True)
            new_item.update({'url': new_item.url.replace(self.portal_item.id, new_item.id)})
