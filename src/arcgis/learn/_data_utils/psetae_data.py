...
class PSATAEDataset(data.Dataset):
    ...
    def __repr__(self):
        ...
        return f"{self.__class__.__name__}, Tensor:{(item[0][0].shape)}, Label:{(item[1])}, items:{self.len }"
...
def create_train_val_sets(path, val_split_pct, working_dir, **kwargs):
    ...
    ntemp = int(kwargs.get("n_temporal"))
    npixels = kwargs.get("min_points")
    ntempdates = kwargs.get("n_temporal_dates")
    use_band_index = kwargs.get("channels_of_interest")
    use_time_index = kwargs.get("times_of_interest")
    ...
    vnt = 1
    for n in label_folds:
        if int(n) in interest_class:
            class_mapping_dict[vnt] = int(n)
            vnt += 1
    ...
    if (
        ...
        for i in label_folds:
            ...
            train_labels.append(file_lst[val_num_images:])
            valid_labels.append(file_lst[:val_num_images])

        train_labels, valid_labels = sum(train_labels, []), sum(valid_labels, [])
        ...
        for i in progress_bar(label_lst):
            ...
            if class_name in interest_class:
                ...
                img_arr, lab_arr = (
                    img_file.data.permute(1, 2, 0),
                    torch.sum(lab_file.data.permute(1, 2, 0), axis=2)[:, :, None],
                )

                concate_arr = torch.cat((img_arr, lab_arr), axis=2)

                timeseries_arr = concate_arr[torch.where(concate_arr[:, :, -1] != 0)][
                    :, :-1
                ]

                c = torch.swapaxes(timeseries_arr, 1, 0)
                nchannel = int(c.shape[0] / ntemp)
                final = torch.reshape(c, (ntemp, nchannel, timeseries_arr.shape[0]))
                ...
                if i in train_labels:
                    ...
                    num_shp = list(range(final.shape[2]))
                    u = 1 + cnt
                    cnt += len(lol1(num_shp, npixels))
                    save_cnt_lst = list(range(u, cnt + 1))

                    cntt = len(list(range(u, cnt + 1)))
                    train_lab_app.append(cntt * [class_name])

                    cnt_lst_t.append(save_cnt_lst)
                    ...
                    save_cnt_lst = list(range(s, cnt1 + 1))

                    cntt = len(list(range(s, cnt1 + 1)))
                    val_lab_app.append(cntt * [class_name])

                    cnt_lst_v.append(save_cnt_lst)
                ...
                min_pix_list.append(final.numpy().shape[2])
                whole_data.append(final.numpy())

        if use_time_index:
            new_ntemp = final.shape[0]
        else:
            new_ntemp = None
        ...
        if npixels == None:
            npixels = int(np.median(min_pix_list))
        ...
        labs_dict = {
            "train": sum(train_lab_app, []),
            "cn_t": len(sum(cn_t, [])),
            "valid": sum(val_lab_app, []),
            "cn_v": len(sum(cn_v, [])),
        }
        labels_dic = {}
        ...
        for k in labs_dict.keys():
            if k in ["train", "valid"]:
                ...
                lab = dict()
                for i, j in zip(items, range(1, len(items) + 1)):
                    lab[str(i)] = int(j)
                re_lst = []
                for z in labs_dict[k]:
                    if str(z) in list(lab.keys()):
                        re_lst.append(lab[str(z)])

            if k == "train":
                Id_with_train_labels = dict()
                for i, j in zip(range(1, labs_dict["cn_t"] + 1), re_lst):
                    Id_with_train_labels[str(i)] = int(j)
                labels_dic["train_labels"] = Id_with_train_labels
            else:
                Id_with_valid_labels = dict()
                for i, j in zip(range(1, labs_dict["cn_v"] + 1), re_lst):
...
def prepare_psetae_data(
    ...
    data._class_map_dict = train_val_dataset[1]

    if class_mapping:
        for r in range(1, len(data._class_map_dict) + 1):
    ...
    data._n_temp = (
        train_val_dataset[6] if train_val_dataset[6] else int(kwargs.get("n_temporal"))
    )
...
def show_batch(self, rows=5, **kwargs):
    ...
    rev_map = {v: k for k, v in self._class_map_dict.items()}
    ...
    for z in range(n):
        class_1_arr = np.concatenate(viz_data_seg[z + 1], axis=2)
        ...
        for j in range(class_1_arr.shape[1]):
            for o, i in enumerate(randlist):
                ...
                if o == 0:
                    ...
                    lab = None
                ax.plot(bands, class_1_arr[:, :, i][:, j], color=colours[j], label=lab)
                ax.legend(loc="upper right")
        ax.set_title(label=str(self._class_map_dict[z + 1]), fontsize=17, pad=10)
    for ax in axs.flat:
