...
class SequenceToSequenceDataBunch(TextDataBunch):
    ...
    @classmethod
    def create(cls, train_ds, valid_ds, test_ds=None, path='.', bs=32, pad_idx=1,
               dl_tfms=None, pad_first=False, no_check=False, 
               backwards=False, val_bs=None, collate_fn=None, **dl_kwargs):
        ...
        return cls(*dataloaders, dl_tfms=dl_tfms, path=path, collate_fn=collate_fn, no_check=no_check, device=device)
...
class SequenceToSequenceLearner(Learner):
    def __init__(self, data,  model, metrics=None, loss_func=None, **learn_kwargs):
        ...
        super().__init__(data, model, metrics=metrics, loss_func=loss_func, **learn_kwargs)
        self.tokenizer = self.model._tokenizer #AutoTokenizer.from_pretrained(pretrained_model_name)


    def show_results(self, ds_type=DatasetType.Valid, rows:int=5, num_beams=4, max_len=50):
        if not HAS_NUMPY:
        ...
        zs = self.model._transformer.generate(x[0], num_beams=num_beams, max_length=max_len)
