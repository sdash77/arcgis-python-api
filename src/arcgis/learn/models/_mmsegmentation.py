...
class MMSegmentationConfig():
    ...
    def on_batch_begin(self, learn, model_input_batch, model_target_batch, **kwargs):
        ...
        image_scale_factor = self.numpy.array([1., 1., 1., 1.], dtype=self.numpy.float32)
        ...
        if not self.model.training:
            if self.cfg.model.type=='CascadeEncoderDecoder':
                ...
                for i in range(self.cfg.model.num_stages):
                    losses += self.model.decode_head[i].losses(model_output, model_target[0])['loss_seg']
                return losses

            return self.model.decode_head.losses(model_output, model_target[0])['loss_seg']
