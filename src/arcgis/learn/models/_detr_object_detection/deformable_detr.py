...
class SetCriterion(nn.Module):
    ...
    def loss_object_embedding_loss(self, outputs, targets, indices, num_boxes):
        """Compute the losses related to the bounding boxes, the L1 regression loss and the GIoU loss
        targets dicts must contain the key "boxes" containing a tensor of dim [nb_target_boxes, 4]
        The target boxes are expected in format (center_x, center_y, h, w), normalized by the image size.
        """
        ...
        target_features = torch.stack(
            [target_features[i][j] for i, j in zip(tgt_idx[0], tgt_idx[1])], dim=0
        )
        return {
            "object_embedding_loss": torch.nn.functional.l1_loss(
                src_features, target_features, reduction="mean"
            )
        }
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            "object_embedding_loss": self.loss_object_embedding_loss,
        }
...
def build(args, classes):
    ...
    if args.object_embedding_loss:
        losses.append("object_embedding_loss")
        weight_dict["object_embedding_loss"] = args.object_embedding_loss_coeff
