...
class _TSLSTM(nn.Module):
    ...
    def forward(self, input_seq):
        ...
        h0 = torch.zeros(2 * self.num_layers, batch_size, self.hidden).to(self.device)
        c0 = torch.zeros(2 * self.num_layers, batch_size, self.hidden).to(self.device)
        out, (h0, c0) = self.lstm(x, (h0, c0))
        h_comb = torch.cat([h0[: self.num_layers], h0[self.num_layers :]], dim=-1)[-1]
