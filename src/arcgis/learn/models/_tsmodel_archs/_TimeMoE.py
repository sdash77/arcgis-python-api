#    Copyright 2025 Time-MoE Authors
...
class TimeMoeConfig(PretrainedConfig):
    ...
    def __init__(
        self,
        input_size: int = 1,
        hidden_size: int = 384,  # 4096,
        intermediate_size: int = 1536,  # 22016,
        horizon_lengths: List[int] = 1,
        num_hidden_layers: int = 12,  # 32,
        num_attention_heads: int = 12,  # 32,
        num_key_value_heads: int = 12,  # None,
        hidden_act: str = "silu",
...
class TimeMoe(TimeMoePreTrainedModel):
    ...
    def __init__(self, config: TimeMoeConfig):
        ...
        self.loss_func = torch.nn.HuberLoss(reduction="none", delta=2.0)
    ...
    def forward(self, x):
        ...
        one_predictions = self.lm_heads[0](self.outputs[0])

        last_step_pred = one_predictions[:, -1:, :].squeeze(-1)

        return last_step_pred
    ...
    def calc_ar_loss(self, predictions, labels, loss_masks, horizon_length):
        ...
        losses = self.loss_func(shift_predictions, shift_labels)
    ...
    def time_moe_loss(self, model_output, labels):
        ...
        if labels is not None:
            ...
            one_loss = self.calc_ar_loss(model_output, labels, loss_masks, 1)
            ar_loss += one_loss
            ...
            if self.apply_aux_loss:
                router_logits = (
                    self.outputs.router_logits if return_dict else self.outputs[-1]
                )
