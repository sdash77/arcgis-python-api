    ...
    def forward(self, x):
        if self.num_frames > 1:
            if x.shape[1] == 6:
                x = (
                    x.unsqueeze(1)
                    .repeat(1, self.num_frames, 1, 1, 1)
                    .view(
                        x.shape[0], x.shape[1], self.num_frames, x.shape[2], x.shape[3]
                    )
                )
            else:
                x = torch.reshape(
...
class GeospatialNeck(nn.Module):
    ...
    def __init__(
        ...
        self.channels = [
            first_conv_channels // (channel_reduction_factor**i)
        ...
        def _build_upscale_block(channels_in, channels_out):
            ...
            layers += [
                ...
                for _ in range(num_convs_per_upscale)
            ]
