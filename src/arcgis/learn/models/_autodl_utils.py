...
self_obj = None
best_name_time = None
all_val_losses, all_train_losses, dice, BestPerformingModel, best_backbone, best_model, timing =  [], [], [], None, None, None, []
...
def generate_output_report(df, output_folder, exhaustive_mode_studies, mode, save_to_folder):       
    content = ""
    header = """
            <!DOCTYPE html>
            <html>
    ...
    '''
    # print(df)
    data_x = list(pd.Series(df.index).astype(str)+"_"+df["Model"] + "_" + df["backbone"])[::-1]
    ...
    for bar in bars:
        plt.text(bar.get_x()+ bar.get_width(), bar.get_y()+bar.get_height()/2, round(bar.get_width(),2), size=14)
    ...
    if accuracy_fn=="average_precision_score":
        df_ = df[['Model', 'train_loss', 'valid_loss', 'average_precision_score', 'lr',
        'training time',"optuna_study" ,'backbone']]
    else:
        df_ = df[['Model', 'train_loss', 'valid_loss', 'accuracy',"dice", 'lr',
        'training time',"optuna_study", 'backbone']]
    ...
    for i, tbc in enumerate(table_body_content, start=1):
        ...
        for j, data in enumerate(tbc, start=1):
            if j == 1:
                ...
                table_content += '''<td style="text-align: left;"><u>
                            <a onclick="toggleShow(\''''+ str(data) + "_"+ tbc[-1] + id_n +'''\');toggleShow('main')" >
                                '''+ str(i) + '''_''' + str(data) +'''
                            </u></a>
                        </td>\n'''
            else:
                if j < len(tbc) - 2:
                    data = round(data, 6)
                table_content += '''<td style="text-align: left;">
                                '''+ str(data)+'''
                        </td>\n'''
    ...
    div_accuracy_chart = '''<h3>AutoDL model's accuracy graph</h3>
    <p><img style="width:1100px" alt="AutoDL Performance" src="data:image/png;base64,'''+ pic_hash.decode('utf-8') +'''" /></p>
    ...
    if mode == "advanced":
        ...
        for i, ex in enumerate(exhaustive_mode_studies):
            ...
            for key, val in best_params.items():
                model_report += '<li><strong>'+ str(key) +'</strong>: '+ str(val) +'</li>'
            ...
            try:
                ...
                model_report += '''
                    <p><img style="width:750px" alt="AutoDL Performance" src="data:image/png;base64,'''+ pic_hash.decode('utf-8') +'''" /></p>
                    '''
        ...
        if save_to_folder:
            ...
            folder_name = "AutoDL_"+ df["Model"][0] + "_" + df["backbone"][0] + "_"+df["timing"][0]
            html_report = os.path.join(output_folder, "models", folder_name, "model_metrics.html")
        ...
        for i, ex in enumerate(exhaustive_dfs):
            ...
            for ind, dl in ex_df.iterrows():
                ...
                hidden_divs += '<div id="'+ str(ids) + str(round(dl["value"],6)).replace(".", "_") +'" style="display: none">'
                hidden_divs += '<h1>Summary of '+ str(name) +'</h1>'
                hidden_divs += '''<p><a onclick="toggleShow(\''''+ str(ids) + str(round(dl["value"],6)).replace(".", "_")+'''\');toggleShow('main')" >&lt;&lt; Go back</a></p>'''
                hidden_divs += '<h2>Model name: <u>' + str(name) +'</u> </h2>'
                hidden_divs += '<h2>Model parameters</h2>'
                hidden_divs += '<ul>'
                for key, val in dl.items():
                    hidden_divs += '<li><strong>'+ str(key) +'</strong>: '+ str(val) +'</li>'
                
                hidden_divs += '</ul>'
                hidden_divs += '<h2>Model Performance Report</h2>'
                folder_name = "AutoDL_"+ name + "_" + dl["params_backbones"] + "_"+ ex._timing[ind]
                html_model_report = os.path.join(output_folder, "models", folder_name, "model_metrics.html")
...
def _objective(trial):
    ...
    model.fit(15, lr=trial.suggest_float("lr", 1e-4, 1e-2), early_stopping=True, callbacks=callbacks, checkpoint=False)
    ...
    if self_obj._save_evaluated_models:
        if self_obj._save_to_folder:
            model.save(
                ...
                + "_"
                + params["backbones"]+"_"+best_name_time
            )
            if self_obj.verbose:
                log_msg = "{date}: model saved at {path}".format(
                    ...
                    path=os.path.join(
                        self_obj._output_path,
                        "models",
                        "AutoDL_" + trial.study.study_name + "_" + params["backbones"]+"_"+best_name_time,
                    ),
                )
                print(log_msg)
                temp_log_msg.append(log_msg)
        else:
            model.save("AutoDL_" + trial.study.study_name + "_" + params["backbones"]+"_"+best_name_time)
            if self_obj.verbose:
                log_msg = "{date}: model saved at {path}".format(
                    date=dt.now().strftime("%d-%m-%Y %H:%M:%S"),
                    path=os.path.join(
...
def _train_exhaust_mode(self, time, name, val_losses, train_losses, d):
    ...
    study.optimize(func=_objective, n_trials=None, show_progress_bar=True, timeout=time, gc_after_trial=True)
    if self_obj.verbose:
