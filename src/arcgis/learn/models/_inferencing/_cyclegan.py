...
class ChildImageClassifier:
    ...
    def updatePixels(self, tlc, shape, props, **pixelBlocks):
        ...
        cyclegan_prediction = util.pixel_classify_cyclegan_image(
            self.model, batch, self.device, self.direction, model_info=self.json_info
        )
        cyclegan_prediction = batch_to_tile(
            cyclegan_prediction.unsqueeze(dim=1).detach().cpu().numpy(),
            batch_height,
            batch_width,
        )
