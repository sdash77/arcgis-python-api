...
class ChildObjectDetector:
    ...
    def vectorize(self, **pixelBlocks):
        ...
        with torch.no_grad():
            batch_output = (
                self.model.learn.model(torch.tensor(batch).to(self.device).float())
            ).cpu()

        num_boxes = 0
        for chip_idx, output in enumerate(batch_output):
            pp_output = self.model._analyze_pred(
                pred=output, thresh=self.thres, nms_overlap=self.nms_overlap
            )
            ...
            if not image_bbox is None:
                for feature_idx in range(len(image_bbox.data[0])):
                    to_append = pred2dict(
                        ((image_bbox.data[0][feature_idx] + 1) / 2).numpy(),
