...
def jenks_matrices(data, n_classes):
    ...
    for l in range(2, len(data)+1):
        ...
        for m in range(1, l+1):
            ...
            # of samples.
            variance = sum_squares - (vsum * vsum) / w
...
def jenks(data, n_classes):
    if n_classes > len(data):
        raise Exception("The number of classes exceeds the amount of data points.")

    data = data.sort_values().reset_index(drop=True)

    lower_class_limits, _ = jenks_matrices(data, n_classes)
    breaks =  get_jenks_breaks(data, lower_class_limits, n_classes)
    min_val = breaks[0]
    max_val = breaks[-1]
    results = []
    for i in range(len(breaks)):
        ...
        min_val = breaks[i]
        max_val = breaks[i+1]
        results.append({"classMinValue": min_val, "classMaxValue": max_val, "label": "%s - %s" % (min_val, max_val)})
