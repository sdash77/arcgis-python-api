...
def _from_xy(df, x_column, y_column, sr=None):
    ...
    for idx, row in df.iterrows():
        geoms.append(
            Geometry({"x": row[x_column], "y": row[y_column], "spatialReference": sr})
        )
    df["SHAPE"] = geoms
...
def from_featureclass(filename, **kwargs):
    ...
    if HASARCPY:
        ...
        sr = kwargs.pop(
            "sr",
            arcpy.Describe(filename).spatialReference or arcpy.SpatialReference(4326),
        )
        ...
        default_polygon = _types.Geometry(
            arcpy.Polygon(arcpy.Array([arcpy.Point(0, 0)] * 3))
        )
        default_polyline = _types.Geometry(
            arcpy.Polyline(arcpy.Array([arcpy.Point(0, 0)] * 2))
        )
        default_point = _types.Geometry(arcpy.PointGeometry(arcpy.Point()))
        default_multipoint = _types.Geometry(
            arcpy.Multipoint(arcpy.Array([arcpy.Point()]))
        )
...
def _arcpy_to_featureclass(
    ...
    if overwrite and arcpy.Exists(os.path.join(out_location, out_name)):
        arcpy.Delete_management(os.path.join(out_location, out_name))
    elif overwrite == False and arcpy.Exists(os.path.join(out_location, out_name)):
    ...
    for idx, a in enumerate(array[0]):
        ...
        elif isinstance(a, (int, np.int32)):
            dtypes.append((flds[idx], np.int64))
        elif isinstance(a, (float, np.float, np.float64)):
            dtypes.append((flds[idx], np.float64))
        elif isinstance(a, DATETIME_TYPES):
    ...
    da.ExtendTable(
        ...
        in_array=array,
        array_match_field="JOIN_ID_FIELD_DROP",
        append_only=False,
    )
...
def to_featureclass(
    ...
    if HASARCPY:
        ...
        max_length = None
        if out_location:
            if os.path.isdir(out_location) == False and out_location.lower().endswith(
                ".gdb"
            ):
                out_location = arcpy.CreateFileGDB_management(
                    out_folder_path=os.path.dirname(out_location),
                    out_name=os.path.basename(out_location),
                )[0]
            elif os.path.isdir(out_location) == False and out_name.lower().endswith(
                ".shp"
            ):
                os.makedirs(out_location)
            elif os.path.isfile(
                out_location
            ) == False and out_location.lower().endswith(".sde"):
        ...
        for col in df.columns:
            if col.lower() != "shape":
                ...
                idx += 1
                col = sanitize_field_name(s=col, length=max_length)
            cols.append(col)
            del col
        df.columns = cols

        if arcpy.Exists(fc) and overwrite:
        ...
        col_insert = [
            ...
            for f in col_insert
            if f.lower()
            not in ["oid", "objectid", "fid", desc.oidFieldName.lower(), af, lf]
        ]
        df_cols = col_insert.copy()
        lower_col_names = [
            f.lower() for f in col_insert if f.lower() not in ["oid", "objectid", "fid"]
        ]
        idx_shp = None
        ...
        if (
            hasattr(desc, "areaFieldName")
            and desc.areaFieldName.lower() in lower_col_names
        ):
            ...
            lower_col_names = [f.lower() for f in col_insert]
        if (
            hasattr(desc, "lengthFieldName")
            and desc.lengthFieldName.lower() in lower_col_names
        ):
        ...
        for col in col_insert:
            if (
                col.lower() != "shape@"
                and col.lower() != "shape"
                and col.lower() not in existing_fields
            ):
                try:
                    ...
                    if t == "TEXT" and out_name.lower().endswith(".shp") == False:
                        ...
                        arcpy.AddField_management(
                            in_table=fc, field_name=col, field_type=t
                        )
                except:
                    print("col %s" % col)
        ...
        return _pyshp_to_shapefile(df=df, out_path=out_location, out_name=out_name)
    else:
        raise Exception(
...
def _geojson_to_esrijson(geojson):
    ...
    if geojson["type"] in ["Polygon", "MultiPolygon"]:
        return {"rings": geojson["coordinates"]}
