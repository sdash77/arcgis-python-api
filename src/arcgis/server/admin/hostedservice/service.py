import json
import collections
from ..._common._base import BasePortal
########################################################################
            ...
            if k == "foldersDetail":
                for item in v:
                    if 'isDefault' in item and item['isDefault'] == False:
                        ...
                        resFolder = self._con.get(path=fURL,
                                                  params=params)
...
class AdminFeatureServiceLayer(BasePortal):
    ...
    def refresh(self):
        ...
        params = {"f": "json"}
        uURL = self._url + "/refresh"
        res = self._con.get(path=uURL, params=params)
        self.init()
        return res
    #----------------------------------------------------------------------
    @property
