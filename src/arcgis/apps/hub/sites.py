...
class Site(OrderedDict):
    ...
    def delete(self) -> bool:
        ...
        if not self._gis._portal.is_arcgisonline:
            # Fetch Enterprise Site Collaboration group if exists
            try:
                _collab_group_id = self.item.properties["collaborationGroupId"]
                ...
                pass
            # Fetch Content Group
            _content_group_id = self.item.properties["contentGroupId"]
        ...
        **Argument**        **Description**
        ...
        if self._gis._portal.is_arcgisonline:
            if self.initiative is None:
                raise Exception(
                    "Sites are created as part of an Initiative for your Hub. Please add a new initiative to proceed."
                )
        ...
        # For Enterprise Sites
            ...
            typekeywords = (
                ...
                + subdomain
                + ", JavaScript, Map, Mapping Site, Online Map, OpenData, Ready To Use, selfConfigured, Web Map"
            )
            ...
            if self._gis.content.search(
                ...
                raise ValueError(
                    "You already have a site that uses this subdomain. Please provide another title."
                )
        ...
        _data = self._create_and_register_site(
            site, subdomain, _site_data, content_group_id, collab_group_id
        )
        ...
        **Argument**        **Description**
        ...
        if self.initiative is not None:
            ...
            if self._hub._hub_enabled:
                _site_properties["properties"][
                    "followersGroupId"
                ] = self.initiative.followers_group_id
            if collab_group_id:
                collab_group = self._gis.groups.get(collab_group_id)
        ...
        # Pages of the site
        site_pages = new_site.pages.search()
        # If pages exist
        if len(site_pages) > 0:
...
class Page(OrderedDict):
        ...
        **Argument**              **Description**
        ...
        if slug:
            ...
            for item in linked_sites:
                ...
                site.definition["values"]["pages"] = [
                    p
                    for p in site.definition["values"]["pages"]
                    if p["id"] != self.itemid
                ]
                _renamed_page = {}
    ...
    def delete(self) -> bool:
        ...
        for item in linked_sites:
            ...
            definition["values"]["pages"] = [
                p for p in site.definition["values"]["pages"] if p["id"] != self.itemid
            ]
...
class PageManager(object):
        ...
        **Argument**               **Description**
        ...
        # For Enterprise Sites
            ...
            description = (
                "DO NOT DELETE OR MODIFY THIS ITEM. This item is managed by the ArcGIS Enterprise Sites application. To make changes to this site, please visit"
                + self._gis.url
                + "/apps/sites/#/home/overview/edit/"
            )
            image_card_url = (
                self._gis.url
                + "/apps/sites/images/placeholders/page-editor-card-image-placeholder.jpg"
            )
        # Create page item
        _item_dict = {
            "title": title,
        ...
        # Fetching page data
        data_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "_store/pages-data.json")
        )
        with open(data_path) as f:
            _page_data = json.load(f)

        # Updating page data
        _page_data["values"]["layout"]["sections"][1]["rows"][0]["cards"][0][
            "component"
        ]["settings"]["src"] = image_card_url
    ...
    def clone(self, page: Page, site: Site | None = None) -> Page:
        ...
        **Argument**        **Description**
        ...
        .. code-block:: python
            ...
            USAGE EXAMPLE: Clone a page to a site successfully
            ...
            #Clone it
            page2 = mySite.pages.clone(page1)
            page2.item
        """
        from datetime import timezone
    ...
    def unlink(self, page: Page, site: Site | None = None) -> bool:
        ...
        **Argument**               **Description**
        ...
        _site_data = site.definition
        _page_data = page.definition
        _site_data["values"]["pages"] = [
            p for p in _site_data["values"]["pages"] if p["id"] != page.itemid
        ]
        _page_data["values"]["sites"] = [
            s for s in _page_data["values"]["sites"] if s["id"] != site.itemid
        ]
