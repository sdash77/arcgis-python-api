...
def update_view(schema, relationships, survey, parent, view_service):
    ''' Update view service definition '''
    form_view_definition = {
                "layers": [],
                "tables": []
                }
    ...
    form_view_flcm = arcgis.features.managers.FeatureLayerCollectionManager(url=form_view_url, gis=survey._gis, fs=view_service)
    try:
...
def add_repeat(survey, service, new_repeat, use_non_globalid_relationships, view_update, view_service):
    ...
    flcm = arcgis.features.managers.FeatureLayerCollectionManager(url=url, gis=survey._gis, fs=service)
    add_repeat = {}
    for dict in new_repeat:
        for layer in dict:
            if layer in add_repeat.keys() and "rel_only" in list(dict[layer].keys()) and dict[layer]['relationships'][0]['name'] in [x for x in add_repeat[layer]['relationships']]:
                pass
            elif layer in add_repeat.keys() and "rel_only" in list(dict[layer].keys()) and dict[layer]['relationships'][0]['name'] not in [x for x in add_repeat[layer]['relationships']]:
                add_repeat[layer]['relationships'].extend(dict[layer]['relationships'])
            else:
                add_repeat.update({layer: dict[layer]})

    schema, relationships = xml2sd(add_repeat, use_non_globalid_relationships)
    ...
    if view_update is True:
        update_view(schema, relationships, survey, service, view_service)

# =============================================================================================================

def init_schema(survey, use_non_globalid_relationships, xform, table_only, create_coded_value_domains, enable_sync):
    ...
    flcm = arcgis.features.managers.FeatureLayerCollectionManager(url=url, gis=survey._gis, fs=survey._ssi)
    xml_schema = xmlschema(xform, "new", survey._gis.properties.isPortal, table_only, use_non_globalid_relationships, create_coded_value_domains)
    
    if not(isinstance(xml_schema, dict)):
        warnings.warn(*xml_schema, sep="\n")
        exit()
    
    schema, relationships = xml2sd(xml_schema, use_non_globalid_relationships)
    ...
    try:
        ...
        flcm.update_definition({"hasStaticData":False,"editorTrackingInfo":{"enableEditorTracking":True,"enableOwnershipAccessControl":True,"allowOthersToUpdate":True,"allowOthersToDelete":True,"allowAnonymousToQuery":False,"allowAnonymousToUpdate":False,"allowAnonymousToDelete":False}, "capabilities":capabilities})
    except AttributeError:
        pass
    
    form_view = survey._gis.content.create_service(name=f"survey123_{'a%s' % uuid.uuid4().hex}_form", folder=survey._si.ownerFolder, is_view=True)
    form_view.update({
        ...
        thumbnail=os.path.join(os.path.abspath(os.path.dirname(__file__)), "assets", "thumbnail.png")
    )
    form_view_definition = {
    "layers": [],
    "tables": []
    ...
    form_view_flcm = arcgis.features.managers.FeatureLayerCollectionManager(url=form_view_url, gis=survey._gis, fs=form_view)
    try:
        form_view_flcm.add_to_definition(form_view_definition)
    except AttributeError:
        pass
...
def modify_schema(survey, service, schema_changes, deltas, use_non_globalid_relationships, update_view = None, view_service = None):
    ...
    if len(layers) > 0:
        if schema_changes is True:
            add_repeat(survey, service, layers, use_non_globalid_relationships, update_view, view_service)
        else:
            layer = [[y for y in x if 'rel_only' not in x[y]] for x in layers]
            [error_reciept['Layer Errors'].append(f"Layer not found in feature service: {x[0]}") for x in layer]
            error += 1
    else:
    ...
    for layer in deltas:
        error_reciept.update({f"{layer} Errors": {
            "Field Errors": [],
            "Field Type Errors": [],
            "Field Length Errors": [],
            "Geometry Errors": []
        ...
        if len(deltas[layer]['fields']) > 0:
            if schema_changes is True:    
                ...
                [error_reciept[f"{layer} Errors"]['Field Errors'].append(f"Field not found in feature service: {x['name']}") for x in deltas[layer]["fields"]]
                error += 1
        else:
        ...
        if len(deltas[layer]['domains']) > 0:
            if schema_changes is True:
                ...
                domains = {"fields": [{"domain":x['schema']['domain'],"name":x['schema']['name']} for x in deltas[layer]['domains']]}
                flm.update_definition(domains)
            else:
                choices = [[y['name'] for y in x['choices']] for x in deltas[layer]['domains']]
                ch = []
                for cl in choices: 
                    for choice in cl:
                        ch.append(choice)
                [warnings.warn(f"Warning: Choice {x} not found in feature service domain") for x in ch]
        if len(deltas[layer]['fieldalias']) > 0 and schema_changes is True:
            # Covers all field alias updates
            alias = {"fields": [{"alias": x['alias'], "name": x['name']} for x in deltas[layer]['fieldalias']]}
            flm.update_definition(alias)
        if len(deltas[layer]['fieldtype']) > 0:
            [error_reciept[f"{layer} Errors"]['Field Type Errors'].append(f"Field {x['fieldname']} in {x['layer']} has a type mismatch: {x['xmltype']} != {x['servicetype']}") for x in deltas[layer]['fieldtype']]           
        else:
            error_reciept[f"{layer} Errors"].pop('Field Type Errors')
        if len(deltas[layer]['fieldlength']) > 0:
            [error_reciept[f"{layer} Errors"]['Field Length Errors'].append(f"Field {x['fieldname']} in {x['layer']} length is too short {x['fieldlength']}") for x in deltas[layer]['fieldlength']]
        else:
            error_reciept[f"{layer} Errors"].pop('Field Length Errors')
        if len(deltas[layer]['geometry']) > 0:
            [error_reciept[f"{layer} Errors"]['Geometry Errors'].append(f"{x['xmllayer']} is a Table, a feature layer is required for {x['xmlgeom']['geometryType']}") for x in deltas[layer]["geometry"] if 'xmllayer' in list(x.keys())]
            [error_reciept[f"{layer} Errors"]['Geometry Errors'].append(f"Incompatible geometry types: {x['xmlgeometry']['geometryType']} does not match {x['servicegeometry']['geometryType']} in {x['xmllayer']}") for x in deltas[layer]["geometry"] if 'servicegeometry' in list(x.keys())]
            error += 1
        else:
...
def get_version():
    ...
    for version in response.json()['secured']['windows64_connect']['versions']:
        return '.'.join('.'.join(response.json()['secured']['windows64_connect']['versions'][version]['url'].split('/')[-1].split('_')[2:5]).split('.')[0:3])
    
# =============================================================================================================

def schema_parity(survey, service, xform, table_only, use_non_globalid_relationships, parent_layer):
    ''' Identify deltas between the XForm and feature service '''
    if use_non_globalid_relationships is None:
        use_non_globalid_relationships = id_relationships(service, parent_layer)    
    ...
    for layer in list(service.layers + service.tables):
        ...
        if 'geometryType' in list(layer.properties.keys()):
            service_schema[layer.properties['name']].update({"geometryType": layer.properties['geometryType']})

    xml_schema = xmlschema(xform, "submission", survey._gis.properties.isPortal, table_only, use_non_globalid_relationships, existing_schema=service_schema)
    ...
    for xml_layer in xml_schema:
        if xml_layer not in list(service_schema.keys()):
            ...
            for relationship in xml_schema[xml_layer]['relationships']:
                parent_layer_relationship = [{x: {'id': xml_schema[x]['id'], 'rel_only': True, 'relationships': [y for y in xml_schema[x]['relationships'] if y['relatedTableId'] == xml_schema[xml_layer]['id']]}} for x in xml_schema if xml_schema[x]['id'] == relationship['relatedTableId']][0]
            ...
            for field in service_schema[xml_layer]['fields']:
                ...
                if 'domain' in list(field.keys()):
                    service_fields[field['name']].update({
                        'domain': field['domain']
                    })
            ...
            for field in xml_schema[xml_layer]['fields']:
                if field['name'] not in list(service_fields.keys()):
                    ...
                    deltas[xml_layer]['fieldalias'].append({"alias": field['alias'], "name": field['name']})
                ...
                elif 'length' in list(field.keys()) and field['length'] is not None and field['length'] > service_fields[field['name']]['length'] and field['type'] == 'esriFieldTypeString':
                ...
                elif 'domain' in list(service_fields[field['name']].keys()) and field['domain'] is not None:
                    domains = {x['code']: x['name'] for x in service_fields[field['name']]['domain']['codedValues']}
                    domain_deltas = {
                        "choices": [],
                        "schema": {
                    ...
                    for domain in field['domain']['codedValues']:
                        if domain['code'] not in list(domains.keys()) and domain['name'] not in list(domains.values()):
                            # New choice -> add new 
                            domain_changes += 1
                            domain_deltas['choices'].append({"name": domain['name'],"code": domain['code']})
                            domain_deltas['schema']['domain']['codedValues'].append({'name': domain['name'], 'code': domain['code']})
                        elif domain['code'] not in list(domains.keys()) and domain['name'] in list(domains.values()):
            ...
            if 'geometryType' in list(xml_schema[xml_layer].keys()):
                if 'geometryType' not in list(service_schema[xml_layer].keys()):
                    deltas[xml_layer]['geometry'].append({
                        "xmllayer": xml_layer,
                        "xmlgeom": xml_schema[xml_layer]
                    })
                elif xml_schema[xml_layer]['geometryType'] != service_schema[xml_layer]['geometryType']:
                    deltas[xml_layer]['geometry'].append({
...
def id_relationships(service, layer):
    ...
    if len(parent_layer.properties['relationships']) > 0:
        ...
        if len(keyfield)> 1:
            ...
            keyfield_type = [x['type'] for x in parent_layer.properties['fields'] if x['name'] == keyfield[0]][0]
...
def xform2webform(xform, portalUrl, connectVersion=None):
    ...
    try:
        ...
        with open(os.path.join(dir_path, xlsx_name + ".webform"), 'w', encoding="utf-8") as fp:
    ...
    for layer in xmldict:
        if "rel_only" in xmldict[layer]:
            ...
            if xmldict[layer]['geometryType'] == 'esriGeometryPoint': 
                """ point """
                pointjson = json.loads(open(os.path.join(os.path.abspath(os.path.dirname(__file__)), "json", "point.json"), 'r').read())
                ...
                if len(xmldict[layer]['relationships']) > 0:
                    ...
                    if useGUID is True and 'uniquerowid' in [x['name'] for x in xmldict[layer]['fields']]:
                ...
                """ line """
                linejson = json.loads(open(os.path.join(os.path.abspath(os.path.dirname(__file__)), "json", "line.json"), 'r').read())
                ...
                """ polygon """
                polygonjson = json.loads(open(os.path.join(os.path.abspath(os.path.dirname(__file__)), "json", "polygon.json"), 'r').read())
            ...
            """ table """
            tablejson = json.loads(open(os.path.join(os.path.abspath(os.path.dirname(__file__)), "json", "table.json"), 'r').read())
...
def append_hidden_questions(ns_dict, f_schema, schema): 
        ...
        if len(field_key) > 0:
            if field_key[-1] in list(f_schema.keys()):
                if f_name not in f_schema[field_key[-1]]:
                    if isinstance(ns_dict[field], dict) and ns_dict[field]['type'] != 'geopoint' and ns_dict[field]['type'] != 'geotrace' and ns_dict[field]['type'] != 'geoshape' and ns_dict[field]['type'] != 'binary':
                        if 'esriFieldType' in list(ns_dict[field].keys()):
                            type = ns_dict[field]['esriFieldType']
                        else:
                            type = field_types[ns_dict[field]['type']]
                        ...
                        if 'parentglobalid' in f_schema[field_key[-1]] or 'parentrowid' in f_schema[field_key[-1]]:
                ...
                append_hidden_questions({'/'.join(field_key): ns_dict[field]}, f_schema, schema)
...
def itext_labels(itext):
    ...
    for node in itext:
        ...
        for label in node:
            itext_labels[node.attrib['lang']].update({label.attrib['id']: label[0].text})
    
    if default is not None:
        return itext_labels[default]
    elif any(system_locale in loc for loc in list(itext_labels.keys())):
        return itext_labels[[lang for lang in list(itext_labels.keys()) if system_locale in lang][0]]
    else:
        return itext_labels[list(itext_labels.keys())[-1]]

# =============================================================================================================

def append_labels(body, ns_dict, itext_labels, model):
    ''' Update field alias' appropriatly '''
    for x in body:
        if re.sub("[{][^}]*[}]", "", x.tag) == "group" or re.sub("[{][^}]*[}]", "", x.tag) == "repeat":
            if re.sub("[{][^}]*[}]", "", x.tag) == "group":
                if len([y for y in list(x) if re.sub("[{][^}]*[}]", "", y.tag) == "repeat"])>0:
                    label = x.find('.//{http://www.w3.org/2002/xforms}label')
                    if itext_labels and 'ref' in label.attrib:
                        alias = itext_labels[re.findall("(?<=')[^']+(?=')", label.attrib['ref'])[0]]
                    else:
                        alias = label.text
                    ns_dict.update({'/'.join(x.attrib['ref'].split('/')[-2:]): alias})
            append_labels(x, ns_dict, itext_labels, model)
        elif re.sub("[{][^}]*[}]", "", x.tag) != "label":
            search_apearance = False
            if 'appearance' in list(x.attrib.keys()) and 'search(' in x.attrib['appearance']:
                search_apearance = True
            q_body = {}
            coded_vals = []
            if len([y for y in list(x) if re.sub("[{][^}]*[}]", "", y.tag) == "label"]) == 0:
            ...
            for y in x:
                if re.sub("[{][^}]*[}]", "", y.tag) == "item":
                    ...
                    for i in y:
                        if re.sub("[{][^}]*[}]", "", i.tag) == "label":
                            if itext_labels and 'ref' in i.attrib:
                                name = itext_labels[re.findall("(?<=')[^']+(?=')", i.attrib['ref'])[0]]
                            else:
                                name = i.text
                    ...
                    if not('{http://esri.com/xforms}fieldType' in model[x.attrib['ref']] and model[x.attrib['ref']]['{http://esri.com/xforms}fieldType'] == 'null') and 'generated_note' not in x.attrib['ref'].split('/')[-1] and 'alias' in list(ns_dict[x.attrib['ref']].keys()):
                        alias = ns_dict[x.attrib['ref']]['alias']
                    elif itext_labels and 'ref' in y.attrib:
                        alias = itext_labels[re.findall("(?<=')[^']+(?=')", y.attrib['ref'])[0]]
                    else:
                        alias = y.text
                    q_body.update({'alias': alias})
            if len(coded_vals) > 0 and search_apearance is False:
                q_body.update({'domain': {
                    'type': 'codedValue', 
		            'name': f"cvd_{x.attrib['ref'].split('/')[-1]}", 
		            'codedValues': coded_vals
                }})
            
            if not('{http://esri.com/xforms}fieldType' in model[x.attrib['ref']] and model[x.attrib['ref']]['{http://esri.com/xforms}fieldType'] == 'null') and 'generated_note' not in x.attrib['ref'].split('/')[-1]:
                update = {**ns_dict[x.attrib['ref']], **q_body}
                ns_dict[x.attrib['ref']] = update

# =============================================================================================================

def append_fields(node, ns_dict, new_dict, parent, model, create_domain, use_GUID):
    ''' Recursivly adds fields to the appropriate layer '''
    if re.sub("[{][^}]*[}]", "", node.tag) != "group" and re.sub("[{][^}]*[}]", "", node.tag) != "label" and not any("generated_note" in y for y in node.attrib['ref'].split('/')) and not ('{http://esri.com/xforms}fieldType' in model[node.attrib['ref']] and model[node.attrib['ref']]['{http://esri.com/xforms}fieldType'] == 'null'):
                if ns_dict[node.attrib['ref']]['type'] == 'binary':
                    ...
                    if 'esriFieldType' in list(ns_dict[node.attrib['ref']].keys()) and ns_dict[node.attrib['ref']]['esriFieldType'] == 'esriFieldTypePointZ':
                    ...
                    if 'esriFieldType' in list(ns_dict[node.attrib['ref']].keys()):
                        ...
                        type = field_types[ns_dict[node.attrib['ref']]['type']]
                    if 'domain' in list(ns_dict[node.attrib['ref']].keys()) and create_domain is True:
                        domain = ns_dict[node.attrib['ref']]['domain']
                        if type != "esriFieldTypeString" and len([x['code'] for x in domain['codedValues'] if isinstance(x['code'], str)])>0:
    ...
    elif re.sub("[{][^}]*[}]", "", node.tag) == "group" and not len([y for y in list(node) if re.sub("[{][^}]*[}]", "", y.tag) == "repeat"])>0:
        for subnode in node:
            if re.sub("[{][^}]*[}]", "", subnode.tag) not in ["repeat", "label", "item", "value"]:
                append_fields(subnode, ns_dict, new_dict, parent, model, create_domain, use_GUID)
                ...
                new_dict.update(gen_schema(subnode[0], subnode.attrib['nodeset'].split('/')[-1], ns_dict, model, create_domain, use_GUID, parent))
    elif len([y for y in list(node) if re.sub("[{][^}]*[}]", "", y.tag) == "repeat"])>0:
        new_dict[parent]['relationships'].append({
        ...
        new_dict.update(gen_schema(node[1], node.attrib['ref'].split('/')[-1], ns_dict, model, create_domain, use_GUID, parent))

# =============================================================================================================

def gen_schema(body, parent, ns_dict, model, create_domain, use_GUID, old_parent = None):
    ...
    for node in body:
        ...
        if not(re.sub("[{][^}]*[}]", "", node.tag) == "group" and len([y for y in list(node) if re.sub("[{][^}]*[}]", "", y.tag) == "repeat"])>0):
            append_fields(node, ns_dict, new_dict, parent, model, create_domain, use_GUID)
        else:
            if use_GUID is True:
                if "uniquerowid" not in [x['name'] for x in new_dict[parent]['fields']]:
                    new_dict[parent]['fields'].append({
...
def xmlschema(xml, mode, isPortal, create_table, use_GUID, create_domains=True, existing_schema = {}):
    ...
    if 'itext' in [re.sub("[{][^}]*[}]", "", x.tag) for x in model]:
        multilingual = itext_labels(model.findall('.//{http://www.w3.org/2002/xforms}itext')[0])

    ns_dict = {}
    for x in model:
        if len(x.attrib) > 1 and 'nodeset' in x.attrib and 'type' in x.attrib:      
            if '{http://esri.com/xforms}fieldType' in x.attrib and x.attrib['{http://esri.com/xforms}fieldType'] == 'null' or x.attrib['nodeset'].split('/')[-1] == 'instanceID' or x.attrib['nodeset'].split('/')[-1] == 'instanceName' or 'generated_note' in x.attrib['nodeset'].split('/')[-1]:
                pass
            else:
                ns_dict[x.attrib['nodeset']] = {'type': x.attrib['type'], 'nodeset': x.attrib['nodeset']}
                if '{http://esri.com/xforms}fieldType' in x.attrib and x.attrib['{http://esri.com/xforms}fieldType'] in ['esriFieldTypeString', 'esriFieldTypeDate', 'esriFieldTypeInteger', 'esriFieldTypeSingle', 'esriFieldTypeDouble', 'esriFieldTypeSmallInteger', 'esriFieldTypePointZ', 'esriFieldTypeGUID']:
                    ns_dict[x.attrib['nodeset']].update({'esriFieldType': x.attrib['{http://esri.com/xforms}fieldType']})
                
                if '{http://esri.com/xforms}fieldLength' in x.attrib:
                    ns_dict[x.attrib['nodeset']].update({'length': int(x.attrib['{http://esri.com/xforms}fieldLength'])})
                elif x.attrib['type'] in ['select1', 'select', 'odk:rank', 'string', 'time', 'barcode'] or 'esriFieldType' in list(ns_dict[x.attrib['nodeset']].keys()) and ns_dict[x.attrib['nodeset']]['esriFieldType'] == 'esriFieldTypeString':
                ...
                if '{http://esri.com/xforms}fieldAlias' in x.attrib:
                    ns_dict[x.attrib['nodeset']].update({'alias': x.attrib['{http://esri.com/xforms}fieldAlias']})

    full_model = {x.attrib['nodeset']: x.attrib for x in model if 'nodeset' in x.attrib}

    append_labels(body, ns_dict, multilingual, full_model)

    
    if mode == "new" and isPortal is True:
        field_len_check = {x.split('/')[-1]: len(x.split('/')[-1]) for x in list(ns_dict.keys())  if len(x.split('/')[-1]) > 31}
        if len(field_len_check) > 0:
            return [f"This survey cannot be published. The field name {x} in {parent} is too long ({field_len_check[x]}) for Portal, must be less than 32 characters" for x in field_len_check]

    schema = gen_schema(body, parent, ns_dict, full_model, create_domains, use_GUID, old_parent = None)
    ...
    append_hidden_questions(ns_dict, f_schema, schema)
        ...
        if len(schema[layer]['relationships']) > 0:
            for idx in range(len(schema[layer]['relationships'])):
                ...
                if schema[layer]['relationships'][idx]['role'] == 'esriRelRoleOrigin':
                    schema[layer]['relationships'][idx].update({'relatedTableId': schema[child]['id']})
                elif schema[layer]['relationships'][idx]['role'] == 'esriRelRoleDestination':
                    schema[layer]['relationships'][idx].update({'relatedTableId': schema[rel_parent]['id']})
                ...
                if name not in list(rel_ids_dict.keys()):
                    ...
                    schema[layer]['relationships'][idx].update({'id': rel_ids_dict[name]})

    if mode == "new" and create_table is False and "geometryType" not in list(schema[parent].keys()):
        schema[parent].update({"geometryType": "esriGeometryPoint"})
    elif mode == "new" and create_table is True and "geometryType"in list(schema[parent].keys()):
        return ["Parameter table_only is set to True but a geometry question was detected in the surveys parent layer"]

    return schema

# =============================================================================================================

def identify_sub_url(xml):
    ...
    if 'submission' in [re.sub("[{][^}]*[}]", "", x.tag) for x in model]:
        sub = True
        submission_url = [x.attrib['action'] for x in model if re.sub("[{][^}]*[}]", "", x.tag) == 'submission'][0]
...
def xls2xform(file_path):
    ''' Convert the XLSForm to XForm '''
    (dir_path, file_name) = os.path.split(file_path)
    xlsx_name = os.path.splitext(file_name)[0]
    # https://www.codegrepper.com/code-examples/python/replace+diacritics+python
    replace_diacritics = unicodedata.normalize('NFD', xlsx_name).encode('ascii', 'ignore').decode("utf-8")
    regex_first_pass = re.sub(r'[ .,;:-]', '_', replace_diacritics)
    regex_file_name = re.sub(r'[^A-Za-z0-9_]+', xlsxregex, regex_first_pass)

    url = 'https://survey123.arcgis.com/api/xls2xform'

    file = {'xlsform': (str(regex_file_name) + ".xlsx", open(file_path, 'rb'), 'application/octet-stream')}
    try:
        ...
        raise RuntimeError("XLSForm conversion failed: " + response_json['result']['message'])
    except:
        raise RuntimeError("There was an error, unable to convert XLSForm. Please contact Esri Technical Support for assistance.")
    else:
        ...
        if 'itemsets' in response_json:
            ...
            with open(os.path.join(dir_path, "media", "itemsets.csv"), 'w', newline='') as itemsets:
