...
class SurveyManager(object):
    ...
    def _create(self, project_name:str, survey_item:Item,
        ...
        return
########################################################################
class SurveyAnalysis(object):
    """
    Provides Access to the Survey's data and reporting structure.

    ====================  ===============================================================
    **Argument**          **Description**
    --------------------  ---------------------------------------------------------------
    survey_item           Required Item. This is the item of type Survey.
    --------------------  ---------------------------------------------------------------
    survey_service_item   Required Item. This is the Feature Layer Collection item
                          associated with the survey
    --------------------  ---------------------------------------------------------------
    survey                Required Survey.  This is the parent survey.
    ====================  ===============================================================
    ...
    _ssi = None
    #----------------------------------------------------------------------
    def __init__(self, survery_item, survery_service_item, survey):
        """Constructor"""
        self._si = survery_item
        self._ssi = survery_service_item
        self._survey = survey
    #----------------------------------------------------------------------
    def __str__(self):
        return "<SurveyAnalysis for {iid}>".format(iid=self._si.title)
    #----------------------------------------------------------------------
    def __repr__(self):
        return self.__str__()
    #----------------------------------------------------------------------
    ...
    def generate_report(self, report_template:Item, where:str="1=1", utc_offset:str="+00:00",
        ...
        **Argument**      **Description**
        ...
        if where == "1=1":
            where = {"where":"1=1"}
        url = "https://survey123.arcgis.com/api/featureReport/createReport/submitJob"
        params = {
            "outputFormat" : "docx",
    ...
    def _check_status(self, res, status_type):
        ...
        status_url = "https://survey123.arcgis.com/api/featureReport/jobs/{jid}/status".format(jid=jid)
...
class Survey(object):
    ...
    _sm = None
    _survey_item = None
    _survey_service_item = None
    #----------------------------------------------------------------------
    def __init__(self, item, sm):
        """Constructor"""
        self._survey_item = item
        self._gis = item._gis
        self._sm = sm
        related = self._survey_item.related_items('Survey2Service', direction='forward')
        if len(related) > 0:
            self._survey_service_item = related[0]
    #----------------------------------------------------------------------
    @property
    def properties(self):
        """returns the properties of the survey"""
        return dict(self._survey_item)
    #----------------------------------------------------------------------
    def __str__(self):
        return "<Survey @ {iid}>".format(iid=self._survey_item.title)
    #----------------------------------------------------------------------
    def __repr__(self):
        return self.__str__()
    #----------------------------------------------------------------------
    ...
    def analysis(self) -> SurveyAnalysis:
        """
        provides access to the reporting and raw data of the `Survey`

        """
        return SurveyAnalysis(self._survey_item,
                              self._survey_service_item,
                              self)
