    ...
    def wrapped(self, *args, **kwargs):
        ...
        # now, if performing analysis using a Web GIS, then access the function referencing remote resources
            ...
            try:
                ...
                raise NotImplementedError(f"'{fn_name}' not available using a Web GIS as the source.")
...
def get_helper_service_url(gis: GIS, service_key: str) -> str:
    ...
    err_msg = f"It appears {service_key} is not available in your Web GIS. Please ensure a {service_key} service is " \
              "defined."
...
def geography_iterable_to_arcpy_geometry_list(geography_iterable: Union[pd.DataFrame, Iterable, Geometry],
                                              geometry_filter: str = None) -> list:
    """
    ...
    # if a list, ensure all child objects are polygon geographies and convert to list of arcpy.Geometry objects
        ...
        for geom in geography_iterable:
            if not isinstance(geom, Geometry):
                raise Exception('The provided geographies in the selecting_geometry list do not appear to all be '
                                'valid.')
        geom_lst = geography_iterable
    ...
    if geometry_filter is not None:
        for geom in geom_lst:
            if geom.geometry_type != geometry_filter:
                raise Exception('geography_iterable geographies must be polygons. It appears you have provided at '
...
def get_spatially_enabled_dataframe(input_object: Union[pd.DataFrame, pd.Series, Geometry, Iterable, np.ndarray],
    ...
    if isinstance(input_object, (Iterable, np.ndarray)) and not isinstance(input_object, pd.DataFrame):
