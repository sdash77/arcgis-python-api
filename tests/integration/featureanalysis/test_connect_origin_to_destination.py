...
class TestCalculateOriginToDestination(unittest.TestCase):
    def test_overwrite(self):
        ...
        for profile in profiles:
            ...
            g2 = Geometry({"x" : -118.14, "y" : 33.81, "spatialReference" : {"wkid" : 4326}})
            ...
            origin1 = gis.content.import_data(sdf1, title=f'dataset_{datetime.datetime.now().microsecond}')
            dest1 = gis.content.import_data(sdf2, title=f'dataset_{datetime.datetime.now().microsecond}')
            ...
            target_item = connect_origins_to_destinations(origins_layer=origin1.layers[0],
            ...
            # perform overwrite
            overwrite = connect_origins_to_destinations(origins_layer=origin1.layers[0],
                                         destinations_layer=dest1.layers[0],
