        ...
        def _construct_featureclass(self):
            ...
            fc = os.path.basename(
                arcpy.CreateUniqueName(
                    base_name="a" + uuid.uuid4().hex[:6] + "a",
                    workspace=arcpy.env.scratchGDB,
                )
            )  #

            return arcpy.CreateRandomPoints_management(
                out_path=arcpy.env.scratchGDB, out_name=fc
            )[0]
            ...
            l = Geometry(
                {
                    "paths": [
                        [
                            [-97.061379999787107, 32.837000000449734],
