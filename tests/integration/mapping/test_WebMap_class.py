...
class Test_WebMap_AGO(unittest.TestCase):
    ...
    def test_WebMap_obj_from_item(self):
        try:
            ...
            search_result = PortalUtils.search_portal_item(
                self.gis, "dinotests_sample_demographic_map", "Web Map"
            )
            if search_result:
            ...
            self.assertIsInstance(
                wm_obj, arcgis.mapping.WebMap, "Cannot create WebMap object from item"
            )
    ...
    def test_create_empty_WebMap_obj(self):
        try:
            ...
            # assert
            self.assertIsInstance(
                wm_obj, arcgis.mapping.WebMap, "Cannot create empty WebMap"
            )
            self.assertTrue(
                hasattr(wm_obj.definition, "baseMap"),
                "Empty WebMap obj is missing baseMap",
            )
    ...
    def test_add_layer_simple_FL(self):
        ...
        try:
            ...
            wm_obj.add_layer(fl)
            self.assertTrue(
                hasattr(wm_obj.definition, "operationalLayers"),
    ...
    def test_add_layer_simple_FL_existing_wm(self):
        ...
        try:
            ...
            search_result = PortalUtils.search_portal_item(
                self.gis, "dinotests_sample_demographic_map2", "Web Map"
            )
            if search_result:
                wm_item = search_result
            else:
            ...
            wm_obj.add_layer(fl)
            self.assertEqual(
                3, len(wm_obj.layers), "Error adding layer to existing web map"
            )
            # endregion
    ...
    def test_save_with_correct_extent(self):
        ...
        try:
            ...
            # save web map
            wmitem = wm_obj.save(
                {"title": self.test_case_name, "snippet": "automation", "tags": "dev"}
            )
    ...
    def test_update_webmap_empty_item_properties(self):
        ...
        try:
            ...
            self.assertEqual(
                len(flc_item.layers), len(wm_obj.definition.operationalLayers)
            )
            ...
            self.assertEqual(
                len(flc_item.layers),
                len(original_layers),
                "Saved Web map does not have 3 layers",
            )
            ...
            self.assertEqual(
                len(flc_item.layers) - 1,
                len(updated_layers),
                "Web map was not correctly updated",
            )
    ...
    def test_add_allsupported_layertypes(self):
        ...
        try:
            ...
            vtitem = PortalUtils.search_portal_item(
                self.gis, "set2_vtpk_worldgreen", "Vector Tile Service"
            )
            if not vtitem:
            ...
            self.assertGreaterEqual(
                len(wm_obj.layers), 4, "NUmber of layers added not equal to 4"
            )
    ...
    def test_add_imageryLayer_builtin_rf(self):
        ...
        try:
            ...
            self.assertEqual(
                wm_obj.layers[0].renderingRule.rasterFunction,
        ...
        try:
            ...
            if hasattr(wm_obj.layers[0].renderingRule, "rasterFunctionArguments"):
                self.assertEqual(
                    wm_obj.layers[
                        0
                    ].renderingRule.rasterFunctionArguments.Raster.rasterFunction,
                    "Hillshade",
                )
            else:
    ...
    def test_author_and_save_new_web_map(self):
        ...
        try:
            ...
            self.assertIsInstance(
                wm_item, arcgis.gis.Item, "save_as() does not return and item"
            )
            self.assertEqual(
                wm_item.type, "Web Map", "item type created by save_as() is not Web Map"
            )
    ...
    def test_editlayers_and_saveas_web_map(self):
        ...
        try:
            ...
            self.assertIsInstance(
                wm_item_new, arcgis.gis.Item, "save_as() does not return and item"
            )
            self.assertEqual(
                wm_item_new.type,
                "Web Map",
                "item type created by save_as() is not Web Map",
            )
    ...
    def test_removelayers_and_saveas_web_map(self):
        ...
        try:
            ...
            self.assertGreater(
                num_layers_original,
                0,
                "Number of layers in webmap is less than 0. Cannot proceed with layer removal testcase",
            )
            ...
            self.assertLess(
                len(wm_obj.layers),
                num_layers_original,
                "Number of layers not less than original after removing layers from webmap",
            )
            ...
            wm_obj_new = WebMap(wm_item_new)
            self.assertEqual(
                len(wm_obj_new.layers),
                num_layers_original - 2,
                "Number of layers in saved out webmap is not 2 less than original",
            )
