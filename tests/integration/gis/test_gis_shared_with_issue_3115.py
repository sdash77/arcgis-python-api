            ...
            if len(items) > 0:            
                ...
                if gis.users.me.role == 'org_admin':
                    ...
                    try:
                        ...
                        temp_account = um.create(username=f"test{dt.microsecond}", 
                                                 password="Funk!0P8p", 
                                                 firstname='fake', 
                                                 lastname='account', 
                                                 email='notreal@esri.com', role='viewer')
                        temp_account.update_role('org_admin')
                        isinstance(temp_account, User)
                        temp_account.reset(password="Funk!0P8p", new_password="Funk!0P9p", new_security_question=0, new_security_answer="Redlands")
                        gis2 = GIS(username=temp_account.username, password="Funk!0P9p", verify_cert=False, set_active=False)
