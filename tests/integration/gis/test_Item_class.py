...
class Test_Item_portal_builtin(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.one_to_one_csv_item = PortalUtils.search_portal_item(cls.gis, "set1_overwrite_HFS_csv2", "CSV")
        if not cls.one_to_one_csv_item:
            ...
            cls.assertIsNotNone(cls.one_to_one_csv_item, "Cannot add csv item for one to one case")

        cls.one_to_one_wfl_item = PortalUtils.search_portal_item(cls.gis, "set1_overwrite_HFS_csv2", "Feature Layer")
        if not cls.one_to_one_wfl_item:
            ...
            cls.assertIsNotNone(cls.one_to_one_wfl_item, "Cannot publish csv item for one to one case")
        #endregion
    ...
    def test_publish_shp(self):
        ...
        try:
            ...
            if sr is not None and len(sr) > 0:
                ...
                file_path = os.path.join(self.qalab_data_path, "packages", zip_package_name)
            ...
            tile_item = publish_output.create_tile_service('Streets_Centerline', 1500000, 40000)
            print(tile_item)
    ...
    def test_overwrite_item(self):
        ...
        try:
            ...
            self.assertEqual(old_item_id, new_item_id, "Item ID is not same after overwriting")
            ...
            self.assertEqual(new_df.shape, downloaded_file_df.shape, "The number of rows cols of csv item not same as updated csv file")
    ...
    def test_overwrite_manyHFS_using_csv_errors(self):
        ...
        try:
            # update csv item
            new_csv_path = os.path.join(self.qalab_cls_path, "overwrite_wfl", "set1_overwrite_manyHFS_csv.csv")
            ...
            self.assertTrue("User cant overwrite this service, using this data" in str(expectedException.exception))
    ...
    def test_overwrite_HFS_using_csv(self):
        ...
        try:
            ...
            self.assertGreater(num_features_after_overwrite, num_features_after_delete, "Overwrite did not add new features")
    ...
    def test_overwrite_HFS_using_fgdb(self):
        ...
        fgdb_item = PortalUtils.search_portal_item(self.gis, "title:set1_Item_overwrite_HFS_fgdb.gdb", "File Geodatabase")
        ...
        wfl_item = PortalUtils.search_portal_item(self.gis, "set1_Item_overwrite_HFS_fgdb", "Feature Service")
        if wfl_item is None:
        ...
        delete_result = flayer.delete_features(where="1=1")
        num_features_after_delete = flayer.query(return_count_only=True)
        self.assertEqual(num_features_after_delete, 0, "Delete_features did not remove all features.")
        # endregion
        ...
        try:
            ...
            self.assertEqual(wfl_item.id, new_wfl_item_id, "Item ID is not same after overwriting")
...
class Test_Item_arcgis_online(unittest.TestCase):
    ...
    def test_publish_vtpk(self):
        ...
        try:
            ...
            if publish_output is None:
                ...
                # validate item's type is vector tile service
                self.assertEqual(publish_output.type, 'Vector Tile Service',
    ...
    def test_publish_sd(self):
        ...
        try:
            # search for spk item
            sr = self.gis.content.search(sd_name, item_type='Service Definition', max_items=1)
            if sr is not None and len(sr) > 0:
                sd_item = sr[0]
                ...
                sd_item = self.gis.content.add({'type': 'Service Definition'}, file_path)
            ...
            if publish_output is None:
                ...
                # validate item's type is vector tile service
                self.assertEqual(publish_output.type, 'Map Service', 'Publishing SD does not create an item '
                                                                     'of type Map Service')
    ...
    def test_download_method_txt_data_nopath(self):
        ...
        try:
            ...
            self.assertIsInstance(chicago_data, str,
                                  "Calling download() on csv item does not return download str path")
            self.assertTrue(chicago_data.endswith("set1_Chicago.csv"),
                            "Download file name does not match known input")
            self.assertGreater(chicago_data_size, 0, "Downloaded file size is not greater than 0")
    ...
    def test_overwrite_item(self):
        ...
        try:
            ...
            new_df = read_csv(new_csv_path)
            downloaded_file_df = read_csv(csv_download_path)
            self.assertEqual(new_df.shape, downloaded_file_df.shape,
    ...
    def test_overwrite_manyHFS_using_csv_errors(self):
        ...
        try:
            ...
            # overwrite the feature layer
            self.assertRaises(RuntimeError, self.one_to_many_csv_item.publish(overwrite=True))
    ...
    def test_overwrite_HFS_using_shp(self):
        ...
        """
        # region delete old service on portal
        old_sr = PortalUtils.search_portal_item(self.gis, "title:set1_overwrite_HFS_shp", "Shapefile")
        if old_sr is not None:
            delete_result = PortalUtils.delete_portal_item(self.gis, old_sr)
            if not delete_result:
        ...
        old_wfl_sr = PortalUtils.search_portal_item(self.gis, "set1_overwrite_HFS_shp", "Feature Service")
        if old_wfl_sr is not None:
            delete_result2 = PortalUtils.delete_portal_item(self.gis, old_wfl_sr)
            if not delete_result2:
                raise unittest.SkipTest('Cannot delete old service output. Skipping test.')
        ...
        try:
            ...
            self.assertTrue(item_update_result, "Calling update on shp item does not return True")
    ...
    def test_overwrite_HFS_using_sd(self):
        ...
        """
        # region delete old service on portal
        old_sr = PortalUtils.search_portal_item(self.gis, "title:set1_overwrite_HFS_sd", "Service Definition")
        if old_sr is not None:
            delete_result = PortalUtils.delete_portal_item(self.gis, old_sr)
            if not delete_result:
        ...
        old_wfl_sr = PortalUtils.search_portal_item(self.gis, "set1_overwrite_HFS_sd", "Feature Service")
        if old_wfl_sr is not None:
            delete_result2 = PortalUtils.delete_portal_item(self.gis, old_wfl_sr)
            if not delete_result2:
                raise unittest.SkipTest('Cannot delete old service output. Skipping test.')
        ...
        try:
            ...
            self.assertTrue(item_update_result, "Calling update on sd item does not return True")
