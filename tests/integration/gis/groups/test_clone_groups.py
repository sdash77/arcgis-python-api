...
class TestCloneGroups(unittest.TestCase):
    ...
    def test_clone_groups(self):
        if (
            self.from_gis.url == self.to_gis.url
            and self.from_gis.users.me == self.to_gis.users.me
        ):
        ...
        self.assertIsInstance(
            cloned_groups[0].result(), Group, "Clone job did not return a Group object."
        )
        self.assertEqual(
    ...
    def test_clone_groups_offline_file_config_defaults(self):
        ...
        fp = self.from_gis.groups.clone(groups=self.source_groups, offline=True)
    ...
    def test_clone_groups_load_offline(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(
                fp, "GroupManager clone did not return a list with one CloningJob."
            )
