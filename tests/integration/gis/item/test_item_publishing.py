...
class Test_Item_publish_file_types(unittest.TestCase):  
    ...
    def test_publish_csv(self):
        ...
        self.assertIsInstance(
            csv_lyr_item,
            Item,
            "Failed to publish CSV item as a featur layer Item."
        )
        self.assertEqual(
            csv_lyr_item.type,
            "Feature Service",
        ...
        if self.gis.version >= [2024, 2]:
            if self.gis.properties.isPortal and not self.gis._is_kubernetes:
                ...
                if not "objectStore" in dstore_types:
                    self.skipTest("Hosted scene layers require an Object Store for publication.")
    ...
    def test_publish_tpk(self):
        ...
        self.assertTrue(
            tile_lyr_item,
            "Failed to publish tile layer from TPK item."
        )
        self.assertIsInstance(
            tile_lyr_item,
            Item,
    ...
    def test_publish_tpkx(self):
        tpkx_package_file = get_web_resource_path(
            relative_path="data/redlands_testcase22.tpkx",
            unique_copy=True
        )
    ...
    def test_publish_flyr_sd(self):
        if self.gis.properties.isPortal:
            self.skipTest("Specific service definition file not able to publish to Kubernetes.")        
        ...
        self.assertTrue(
            svcdef_flyr_item,
            "Failed to publish feature layer from service definition."
        )
        self.assertIsInstance(
            svcdef_flyr_item,
            Item,
    ...
    def test_publish_tlyr_sd(self):
        if self.gis.properties.isPortal:
            self.skipTest("Specific service definition publishes to ArcGIS Online only.")
