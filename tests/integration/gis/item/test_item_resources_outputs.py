...
class Test_Item_resources_methods(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.smap_items = cls.gis.content.search(
            query="Giraffes *", item_type="StoryMap"
        )
        if not cls.smap_items:
            try:
                cls.smap_item = cls.gis.content.search(
                    query=f"owner:{cls.gis.users.me.username}", item_type="StoryMap"
                )[0]
                ...
                cover.summary = (
                    "StoryMap to test Python API ResourceManager in Kubernetes."
                )
    ...
    def test_resources_property(self):
        ...
        self.assertGreater(
            len(res_mgr.list()), 0, "StoryMap item has no resources as expected."
        )
        self.assertIsInstance(
            res_mgr.list()[0],
            dict,
