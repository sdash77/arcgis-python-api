...
class TestUpdateThumbnail(unittest.TestCase):
    def test_update_thumbnail_url(self):
        ...
        for profile in profiles:
            ...
            assert res
            assert item.delete(force=False, dry_run=False)
            del gis, res

    def test_update_thumbnail_base64(self):
        """base64 image"""
        import json
        ...
        for profile in profiles:
            ...
            res = item.update_thumbnail(
                encoded_image=base64_img, file_name="thumbnail.png"
            )
            assert res
            assert item.delete(force=False, dry_run=False)
            del gis, res

    def test_update_thumbnail_fp(self):
        """file path"""
        import tempfile
