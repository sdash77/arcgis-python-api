...
class Test_DatastoreManager_portal_builtin(unittest.TestCase):
        ...
        if self._testMethodName == "test_geoanalyticsDM_add_BDFS_winUNC":
            ...
            try:
                ...
                search_list = [
                    ds
                    for ds in DS_list
                    if ds.datapath.__contains__("unittest_fortune500")
                ]
    ...
    def test_is_supported(self):
        ...
        try:
            ...
            self.assertTrue(
                geoanalytics.is_supported(self.gis), "GIS does not support geoanalytics"
            )
            ...
            self.fail(
                "Error during test, most likely cannot import geoanalytics: "
    ...
    def test_gis_get_datastores(self):
        ...
        try:
            ...
            self.assertGreaterEqual(
                len(DM_list), 1, "gis._datastores returns less than 1 DM obj"
            )
    ...
    def test_hostingDM_search_noparams(self):
        ...
        try:
            ...
            self.assertIsNotNone(
                DS_list,
                "Calling search() on hosting server DataStoreManager returns NOne",
            )
            self.assertGreaterEqual(
                len(DS_list), 1, "No Datastore objs were returned by search() method"
            )
    ...
    def test_geoanalyticsDM_search_noparams(self):
        ...
        try:
            ...
            DS_list = ga_DM.search()
            self.assertIsNotNone(
                DS_list, "Calling search() on ga server DataStoreManager returns NOne"
            )
            self.assertGreaterEqual(
                len(DS_list), 1, "No Datastore objs were returned by search() method"
            )
    ...
    def test_geoanalyticsDM_add_BDFS_winUNC(self):
        ...
        try:
            ...
            self.assertGreaterEqual(
                len(search_list), 1, "Newly added BDFS cannot be searched"
            )

            print(
                "Added BDFS can be successfully searched using DatastoreManager.search()"
            )
