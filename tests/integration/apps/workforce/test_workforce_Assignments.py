...
class Test_Workforce_Assignments_With_Assignments(unittest.TestCase):
    ...
    def test_batch_update_assignment(self):
        try:
            ...
            for assignment in assignments:
                self.assertEqual(assignment.status, "assigned", "Incorrect status")
                self.assertEqual(
                    assignment.worker.id, self.worker.id, "Incorrect worker"
                )
    ...
    def test_comp_assignment_properties(self):
        try:
            ...
            self.assertEqual(
                ...
                datetime.datetime(2018, 4, 15).date(),
                "Incorrect assigned date",
            )
            self.assertEqual(
                assignment.completed_date.date(),
                datetime.datetime(2018, 4, 21).date(),
                "Incorrect completed date",
            )
            self.assertEqual(
                assignment.declined_comment, None, "Incorrect declined comment"
            )
            self.assertEqual(assignment.declined_date, None, "Incorrect declined date")
            self.assertEqual(
                assignment.description, "test description", "Incorrect description"
            )
            self.assertEqual(
                assignment.due_date.date(),
                datetime.datetime(2018, 4, 22).date(),
                "Incorrect due date",
            )
            self.assertEqual(
                assignment.in_progress_date.date(),
                datetime.datetime(2018, 4, 18).date(),
                "Incorrect in progress date",
            )
            self.assertEqual(assignment.location, "here", "Incorrect location")
            self.assertEqual(
                assignment.paused_date.date(), datetime.datetime(2018, 4, 19).date()
            )
    ...
    def test_decl_assignment_properties2(self):
        try:
            ...
            self.assertEqual(
                assignment.declined_comment, "declined", "Incorrect declined comment"
            )
            self.assertEqual(
                assignment.declined_date.date(),
                datetime.datetime(2018, 4, 18).date(),
                "Incorrect declined date",
            )
            self.assertEqual(
                assignment.description, "test description", "Incorrect description"
            )
...
class Test_Workforce_Assignments_No_Assignments(unittest.TestCase):
    ...
    def test_batch_add_assignments(self):
        try:
            ...
            assignments = self.project.assignments.batch_add(
