...
class TestAGOLNotebookManager(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls._gis = cls.gis
        if cls.gis._is_agol:
            notebookRuntimeVersion = cls.gis.notebook_server[0].runtimes.properties[
                "currentRuntimeVersion"
            ]
        else:
            notebookRuntimeVersion = (
                cls.gis.notebook_server[0].notebooks.runtimes[0].properties["version"]
            )

        # Define notebook json
        notebook_json = {
            "cells": [
                {
                    "cell_type": "markdown",
        ...
        # Add the notebook item
        folder = cls._gis.content.folders.get()
        job = folder.add(item_properties=item_properties, file=fp)
        cls._item = job.result()
