...
class Test_ImageryLayer_portal(unittest.TestCase):
    ...
    def test_raster_mod_classes(self):
        try:
            ...
            search_result = PortalUtils.search_portal_item(self.gis, "ImgSrv_Landast_Montana2014", "Image Service")

            montana_img_lyr = search_result.layers[0]

            #assert
            self.assertIsInstance(montana_img_lyr,arcgis.raster.ImageryLayer, "Layers property of Img Service item does not return ImageryLayer obj")
    ...
    def test_histograms(self):
        try:
            ...
            self.assertEqual(len(counts_b1), hist_b1['size'], "length of counts list should be equal to size kvp")
