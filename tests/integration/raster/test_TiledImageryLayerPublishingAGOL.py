# -------------------------------------------------------------------------------
# Name:        Tiled Imagery Layer publishing (AGOL) tests
...
@integration_test
class Test_TiledImageryPublishing(unittest.TestCase):
    """
    Publish Tiled Imagery Layer from local files
    """
    ...
    def setUpClass(cls):
        ...
        print("==================================================================")
        print("Beginning tests in Test_TiledImageryPublishing class")
        # endregion

    def setUp(self):
        test_skip = False  # reset the skip flag
        print("Test: " + self._testMethodName)
        self.namePrefix = "Tiled_ImgLyr_"
    ...
    def test_image_mosaic_publishing(self):
        try:

            # Test data (amberg tif)
            data_path = os.path.join(self.qalab_data_path, "090160")

            print(arcgis.__file__)

            img_lyr_item = copy_raster(
                input_raster=data_path,
                raster_type_name="Raster Dataset",
                output_name=self.test_case_name,
                gis=self.gis,
            )

            print("Copy Raster job completed")

            self.assertIsInstance(img_lyr_item, arcgis.gis.Item)

            img_lyr = img_lyr_item.layers[0]
            ...
            nd_array = img_lyr.export_image(f="numpy_array")
            ...
            # delete the item
            self.assertTrue(img_lyr_item.delete())
