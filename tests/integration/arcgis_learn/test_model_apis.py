...
RESULT_FILE = "arcgis_learn_api_test_results.html"
CC_FILE = "arcgis_learn_api_cc_list.txt"
FAILED_TESTS = []
...
def modelAPIs_backbones(
    ...
    d_path = os.path.join(
        ...
        data_path,
        "models",
        model_test + "_" + urllib.parse.quote(bbone, safe=""),
    )
    if bbone == "gradcam":
    ...
    if regression_parameter == "confusion_matrix":
        ...
        print(
            "accuracy value for ", model_test, "is: ", result
        )  # add results to dict later
    elif regression_parameter == "average_precision_score":
        result = model_object.average_precision_score()
        result = [
            v for k, v in sorted(result.items(), key=lambda item: item[1], reverse=True)
        ][0]
    ...
    model_object.load(
        str(model_save_path)
        + os.sep
        + f"{model_test}_{urllib.parse.quote(bbone, safe='')}.emd"
    )
    if bbone == "gradcam":
        print(
            "making gradcam prediction on image at path: ",
            os.path.join(data_folder, data_path, "images", "000000000001.tif"),
        )
        predicted_class = model_object.predict(
            os.path.join(data_folder, data_path, "images", "000000000001.tif"),
            visualize=True,
            gradcam=True,
        )
...
def backboneTestCases(
    ...
    try:
        ...
        if bbone == "gradcam":  # for gradcam
            ...
            if is_ms:
                if model_test == "deeplab_test_ms":
                    ...
                    print(
                        "The ms model initialized with wavelength values from emd file will be",
                        model_test,
                        bbone,
                    )
                else:
                    model_object = model(
                        data, backbone=bbone, wavelengths=wavelengths_dofa
                    )
                    print(
                        "The ms model initialized will be",
                        model_test,
                        bbone,
                        wavelengths_dofa,
                    )
                ...
                print(
                    "The rgb model initialized with custom wavelength values will be",
                    model_test,
                    bbone,
                    wavelengths_dofa,
                )
        ...
        record_failure(model_test, owner)
        raise e
