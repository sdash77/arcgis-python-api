...
def test_get_subgeographies_zips(usa_instance):
    ...
    zips_valid_lst = [isinstance(zip_code, NamedArea) for zip_code in [zip1, zip2, zip3]]
...
def test_merge_on_zip(usa_instance, analysis_variables, health_facility_df):
    ...
    merged = pd.merge(enrich_df, health_facility_df, left_on='StdGeographyID', right_on='Zip Code')
