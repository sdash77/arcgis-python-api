...
def get_countries_test(gis: Union[str, GIS] = None) -> None:
    cntrs = get_countries(gis)
    assert cntrs


def enrich_iterable_str_addresses_test(gis: Union[str, GIS] = None) -> None:
...
def enrich_iterable_dict_addresses_test(gis: Union[str, GIS] = None) -> None:
    er = enrich(address_list_json, analysis_variables=key_enrich_list_web, gis=gis)
    assert isinstance(er, pd.DataFrame)
    assert er.spatial.validate()


def enrich_iterable_geometry_test(gis: Union[str, GIS] = None) -> None:
...
def enrich_iterable_buffer_study_areas_test(gis: Union[str, GIS] = None) -> None:
...
def enrich_sedf_test(gis: Union[str, GIS] = None) -> None:
