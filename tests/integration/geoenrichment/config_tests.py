...
if _agol_url and _agol_user and _agol_pass:
    agol = GIS(os.getenv('AGOL_URL'), username=os.getenv('AGOL_USERNAME'), password=os.getenv('AGOL_PASSWORD'))
    _src_lst.append(agol)
    _src_nm_lst.append('agol')
else:
    warn('Cannot test ArcGIS Online because AGOL_URL, AGOL_USERNAME, and AGOL_PASSWORD are not in the environment '
         'variables.')
