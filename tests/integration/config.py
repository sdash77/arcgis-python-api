...
WEB_RESOURCE_ROOT_PATH = os.environ.get('GEOSAURUS_RESOURCES_WEB_ROOT', 'https://esri-forge.python.geocloud.com/_/data')
...
def get_resource_path(relative_path, verify=True, unique_copy=False):
    ...
    if unique_copy:
        resource_copy = resource.with_name(f"{uuid.uuid4().hex}{resource.suffix}")
        shutil.copy(resource, resource_copy)
...
def get_web_resource_path(relative_path, unique_copy=False):
    ...
    try:
        cached_resource = get_resource_path(_resource_cache_path, verify=True, unique_copy=unique_copy)
