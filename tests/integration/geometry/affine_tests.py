...
class AffineTest(unittest.TestCase):
    ...
    def test_skew(self):
        ...
        for shape in self.shapes:
            ...
            val_geom = isinstance(skew(Geometry(shape), x_angle=45, y_angle=-20), Geometry)
            res.append(val_dict)
            res.append(val_geom)
        self.assertTrue(all(res))
    ...
    def test_scale_on_geom(self):
        ...
        for shape in self.shapes:
            g = Geometry(shape)
            res.append(isinstance(g.scale(.10,.20), Geometry))
            res.append(isinstance(g.scale(.10,.10, True), Geometry))
        self.assertTrue(all(res))
    ...
    def test_translate_on_geom(self):
        ...
        for shape in self.shapes:
            ...
            res.append(isinstance(g.translate(x_offset=3, y_offset=3, inplace=True), Geometry))
        self.assertTrue(all(res))
