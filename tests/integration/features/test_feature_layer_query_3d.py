...
class TestQuery3DFeatureLayer(unittest.TestCase):
    ...
    def test_query_out_fields(self):
        ...
        fields = self.ned_3do_layer.query_3d(
            where="objectid < 5", 
            out_fields=["esri3do_ox", "esri3do_oy", "esri3do_oz"]
        )
    ...
    def test_query_order_by_fields(self):
        ...
        assert ordered
        assert ordered["features"][0]["attributes"]["esri3do_oz"] <= ordered["features"][1]["attributes"]["esri3do_oz"]
        assert ordered["features"][-2]["attributes"]["esri3do_oz"] <= ordered["features"][-1]["attributes"]["esri3do_oz"]

    def test_query_all_records(self):
        """
        Test query with return_all_records=False. ! In this case we only have 94 features...
        """
        limit_records = self.ned_3do_layer.query_3d(
            where="OBJECTID < 43"
        )
    ...
    def test_query_historic_moments_and_time(self):
        ...
        if self.ned_3do_layer.properties["isDataArchived"]:
            start_moment = self.ned_3do_layer.properties["archivingInfo"]["startArchivingMoment"]
         
        historic = self.ned_3do_layer.query_3d(
            where="1=1", 
            historic_moment=start_moment)
    ...
    def test_query_geometry_filter(self):
        ...
        geom_filter = self.ned_3do_layer.query_3d(
            geometry_filter=geom_filt
        )        
        
        assert geom_filter["spatialReference"]["wkid"] == 28992
        assert len(geom_filter["features"]) < len(self.ned_3do_layer.query_3d()["features"])
