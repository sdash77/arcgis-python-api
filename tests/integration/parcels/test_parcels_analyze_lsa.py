...
class TestAnalyzeLSA(unittest.TestCase):
    ...
    def test_analyze_consistency_check_with_parcel_features_async(self):
        ...
        with self.vms.get(fq_version_name, "read"):
            is_async = True
            result = self.parcelFabric.analyze_least_squares_adjustment(
            ...
            if result["success"]:
                ...
                for message in messages:
                    ...
                    if message["description"].startswith("Chi-Square test (95.0%):"):
                        self.assertTrue("FAILED" in message["description"])
            else:
                    ...
                    for k, v in adds[i]["attributes"].items():
                        if k == "AdjustedStdDev":
                            if v is None:
                                continue
                            self.assertTrue(
                                (6.460 < float(v) <= 6.636, 6.46094508),
                ...
                for message in messages:
                    if message["description"].startswith("Chi squared:"):
                        actual = message["description"][
                            message["description"].index(": ") + 1 :
                        ].strip()
                        self.assertAlmostEqual(59.71, float(actual), delta=0.5)

                    if message["description"].startswith(
                        "Global (Pelzer) Reliability:"
                    ):
        ...
        with self.vms.get(fq_version_name, "read"):
            ...
            if result["success"]:
                ...
                for i in range(len(adds)):
                    for k, v in adds[i]["attributes"].items():
                        if k == "ZUncertainty":
                            self.assertEqual(v, 0.03215217, "ZUncertainty is incorrect")
            else:
                self.fail("An error occurred running Analyze LSA")
    ...
    def test_analyze_weighted_LSA_with_parcel_features_async(self):
        ...
        with self.vms.get(fq_version_name, "read"):
            is_async = True
            result = self.parcelFabric.analyze_least_squares_adjustment(
                analysis_type="WEIGHTED_LEAST_SQUARES",
                convergence_tolerance="0.05",
                parcel_features=parcel_features,
                future=is_async,
            )
