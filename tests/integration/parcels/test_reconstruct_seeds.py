...
class TestShrinkReconstructSeeds(unittest.TestCase):
    ...
    def test_ShrinkToSeedsThenReconstruct(self):
        ...
        shrink_to_seeds = parcels_fl.edit_features(updates=parcel_features, gdb_version=fq_version_name)
        self.assertEqual(5, len(shrink_to_seeds["updateResults"]), "Incorrect update count")

        is_seed = pfutils.query_service(self.service_urls["FeatureServer"],
                                         self.gis, fl_id=15, out_fields=["OBJECTID"],
                                         where="IsSeed = 1", version_name=fq_version_name)
        ...
        self.assertEqual(5, rcfs["reconstructedParcelCount"], "Incorrect reconstructedParcelCount value")
