...
class AddLayerUnitTest(unittest.TestCase):
    @patch("arcgis.widgets.MapView", MockMapView)
    def test_add_item_map_service(self):
        """
    ...
    def test_add_featurecollection_different_renderers(self):
        ...
        mock_webmap.add_layer = lambda layer, options: WebMap.add_layer(
            mock_webmap, layer, options
        )

        placeholder_fl_1 = PlaceholderFeatureLayer("https://example.com")
        placeholder_fl_2 = PlaceholderFeatureLayer("https://example.com")
        renderer_1 = {"type": "simple", "symbol": {"type": "esriPMS"}}
        renderer_2 = {"type": "simple", "symbol": {"type": "esriSFS"}}
