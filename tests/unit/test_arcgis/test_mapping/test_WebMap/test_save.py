...
class TestSave(unittest.TestCase):
    ...
    def test_process_extent_called_correctly(self):
        ...
        expected_item_prop = {**gen_full_item_prop, **{"extent": validation_extent}}

        WebMap.save(mock_webmap, gen_input_item_prop)
        mock_webmap._gis.content.add.assert_called_with(
            expected_item_prop, folder=None, metadata=None, owner=None, thumbnail=None
        )

    _mock_project_resp_1 = [
        {"x": "NaN", "y": "NaN"},
        {"x": 116.4447558099451, "y": 87.78601210184834},
    ]

    import arcgis.geometry

    @patch("arcgis.geometry.project", return_value=_mock_project_resp_1)
    def test_process_extent_non_4326_wkid_project_1(self, mock_project):
        self._test_process_extent_non_4326_wkid_project("")

    _mock_project_resp_2 = [{"x": "NaN", "y": "NaN"}, {"x": "NaN", "y": "NaN"}]

    @patch("arcgis.geometry.project", return_value=_mock_project_resp_2)
    def test_process_extent_non_4326_wkid_project_2(self, mock_project):
        self._test_process_extent_non_4326_wkid_project("")
    ...
    def test_process_extent_non_4326_wkid_project_3(self, mock_project):
        self._test_process_extent_non_4326_wkid_project(
            "116.4447558099451,87.78601210184834,124.123,81.34"
        )

    def _test_process_extent_non_4326_wkid_project(self, expected_value):
        ...
        mock_webmap._contains_nans = lambda result: WebMap._contains_nans(
            mock_webmap, result
        )
