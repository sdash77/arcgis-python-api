...
SAMPLE_GEOMETRY_HASZ_HASM = {
    ...
    "paths": [
        [
            [11559865.5434, 147204.92960000038, 0, 4],
            [11559866.5434, 147205.92960000038, 0, 4.1],
        ]
    ],
...
SAMPLE_GEOMETRY_HASZ_ONLY = {
    ...
    "paths": [
        [
            [11559865.5434, 147204.92960000038, 0],
            [11559866.5434, 147205.92960000038, 10],
        ]
    ],
    "spatialReference": {"wkid": 102100, "latestWkid": 3857},
}
...
SAMPLE_GEOMETRY_HASM_ONLY = {
    "hasZ": False,
    "hasM": True,
    "paths": [
        [
            [11559865.5434, 147204.92960000038, 1],
            [11559866.5434, 147205.92960000038, 1.1],
        ]
    ],
    "spatialReference": {"wkid": 102100, "latestWkid": 3857},
}
...
class TestGeoAccessorSpatialHasMHasZ(unittest.TestCase):
    ...
    def test_does_not_have_z_m(self):
        data = {"SHAPE": [GEOMS[3]] * 4, "OID": [1, 2, 3, 4]}
        sdf = pd.DataFrame(data)
...
class TestSeDFIOCheckZCheckM(unittest.TestCase):
    ...
    def test_io_featureclass_zm_2(self):
        ...
        data = {"SHAPE": [GEOMS[2]] * 4, "OID": [1, 2, 3, 4]}
        sdf = pd.DataFrame(data)
        sdf.spatial.name
        fc = sdf.spatial.to_featureclass(
            os.path.join("in_memory", "hasmztest"), has_z=True, has_m=False
        )
    ...
    def test_io_featureclass_zm_1(self):
        ...
        sdf.spatial.name
        fc = sdf.spatial.to_featureclass(
            os.path.join("in_memory", "hasmztest"), has_z=False, has_m=False
        )
        desc = arcpy.da.Describe(fc)
        assert desc["hasM"] == False
    ...
    def test_io_featureclass_zm_3(self):
        ...
        data = {"SHAPE": [g] * 4, "OID": [1, 2, 3, 4]}
        sdf = pd.DataFrame(data)
        sdf.spatial.name
        fc = sdf.spatial.to_featureclass(
            os.path.join("in_memory", "hasmztest"), has_z=False, has_m=True
        )
        desc = arcpy.da.Describe(fc)
    ...
    def test_io_featureclass_zm_4(self):
        """tests that the `to_featureclass` where has_z=True and has_m=False"""
        data = {"SHAPE": [GEOMS[0]] * 4, "OID": [1, 2, 3, 4]}
        sdf = pd.DataFrame(data)
        sdf.spatial.name
        fc = sdf.spatial.to_featureclass(
            os.path.join("in_memory", "hasmztest"), has_z=False, has_m=True
