...
def _parse_cmd_line_args():
    parser = argparse.ArgumentParser(
        description="User cmd line tool "
    ...
    parser.add_argument(
        ...
        help="Output directory to write junit xml etc. files to. " "DEFAULT: .\_output",
    )
    parser.add_argument(
        "--no-browser-output",
        "-q",
    ...
    if not any([args.suite, args.tests]):
        raise Exception(
            "You must specify either positional argument `tests`,"
...
def _setup_output_dir(output_dir):
    output_executed_notebooks_dir = os.path.join(output_dir, "executed_notebooks")
...
def _run_tests(args):
    ...
    _parse_suite(suite)
    output_xml_files, output_coverage_files = run_suite(
        suite,
        args.output_dir,
        run_setup_env=False,
    ...
    if not args.no_browser_output:
        _display_results_in_browser(
            output_xml_files, output_coverage_files, args.output_dir
        )
...
def _display_results_in_browser(output_xml_files, output_coverage_files, output_dir):
    try:
        ...
        for output_xml_file in [x for x in output_xml_files if x]:
            ...
            run_shell_command(
                f'"{sys.executable}" "{junit2html_path}" '
