...
def configure_imports(
    __import__real, modules_to_raise_importerrors=[], modules_to_return_magicmocks=[]
):
...
def __import__custom(*args, **kwargs):
    ...
    if ___import__real == None:
        raise Exception(
            "You must call `configure_imports()` and pass in the "
            "the real builtins.__import__ before monkeypatching "
            "this __import__custom() func to builtins.__import__"
        )
    name = args[0]
    if any([name.split(".")[0] == mod for mod in _modules_to_raise_importerrors]):
        raise ImportError(f"module {name} monkeypatched to raise ImportError")
    elif any([name.split(".")[0] == mod for mod in _modules_to_return_magicmocks]):
        return MagicMock()
    else:
