...
def compare_images(img1_path, img2_path, max_perc_diff=0.005):
    ...
    if diff_ratios > 0.05:
        ...
        raise UnsimilarImageError(
            "Ratios are not the same for images, " "cannot compare, failing!"
        )
    ...
    # Since we finally have all the information we need, do the comparison
    if perc_diff > max_perc_diff:
        msg = (
            f"Images are {printable_perc_diff}% different, which is "
