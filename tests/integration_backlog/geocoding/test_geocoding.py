...
class TestAnalyzeGeocodingInput(unittest.TestCase):
    ...
    def test_analyze_table_item(self):
        ...
        item_name = f"geocode_analyze_table_{uid}"
        fp = os.path.join(tempfile.gettempdir(), f"{item_name}.csv")
        df.to_csv(fp)

        gis = self.gis
        csv_item = add_source_item(gis, item_name, ItemTypeEnum.CSV, fp)
        self.items_to_delete.append(csv_item)
        ...
        if gc:
            res = analyze_geocode_input(
                ...
                input_table_or_item={"itemid": csv_item.itemid},
...
class TestSuggestion(unittest.TestCase):
    ...
    def test_extent1(self):
        s1a = suggest(
            text="starbu",
            search_extent="-104,35.6,-94.32,41",
            max_suggestions=1,
        )
        assert s1a
        assert "suggestions" in s1a
