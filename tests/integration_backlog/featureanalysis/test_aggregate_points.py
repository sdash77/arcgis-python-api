...
class TestAggregatePoints(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.restaurant_data = get_json_resource(
            relative_path="/features/test_fa_aggregate_points.json"
        )
        ...
        cls.restaurants_source = cls.restaurants_item.related_items("Service2Data", "forward")[0]
    
        # Publicly avaiable USA Census Block Group Boundaries: esri data
        ...
        if not cls.census_blks_lyr:
            unitest.skip("No polygon layer to aggregate points for.")
                
    @classmethod
    def tearDownClass(cls):
        cleanup_published_items(items=[
            cls.restaurants_item
        ])
    ...
    def test_aggregate_with_group_by(self):
        self.assertEqual(self.restaurants_lyr.properties.geometryType, "esriGeometryPoint", "Feature layer is not point geometry.")
        ...
        self.assertIsInstance(agg_results_item, Item, "Output when specifying output_name not an item.")
        self.assertIsInstance(agg_results_item.layers[0], FeatureLayer, "Item layers output is not a FeatureLayer object.")
        self.assertEqual(len(agg_results_item.layers[0].query().features), 19, "Incorrect number of records returned, should be 19.")
        self.assertTrue(agg_results_item.tables, "Summary table not created as expected.")
        self.assertIsInstance(agg_results_item.tables[0], Table, "Item tables output is not a Table object.")
    ...
    def test_overwrite(self):
        ...
        overwrite_agg_item = aggregate_points(
            ...
            output_name=agg_results_item,
            context={
                "overwrite": True
            }
        )
        self.assertIsInstance(overwrite_agg_item, Item, "Operation with overwrite context did not output item.")
        self.assertEqual(agg_results_item.id, overwrite_agg_item.id, "Item id did not remain the same with overwrite")
        # overwrite should not append. Only one layer and one table should be present
        self.assertEqual(len(overwrite_agg_item.layers), 1, "Overwritten item should have same number of layers as original.")
        self.assertEqual(len(overwrite_agg_item.tables), 1, "Overwritten item should have same number of tables as original.")
