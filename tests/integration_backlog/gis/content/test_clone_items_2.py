...
class TestCloneItems(unittest.TestCase):
    ...
    def test_portal_to_agol_with_preserve_id(self):
        """tests cloning from Portal to AGOL using preserve item id"""
        gis_objs = {
            "enterprise": GIS(profile=profiles[1], verify_cert=False, trust_env=True),
        }
        ...
        item = csource.add(
            item_properties={
                "title": "clone_item_test_" + f"{int(time.time())}"[:10],
        ...
        saved_map_item_id = saved_map.itemid
        gis_dest = GIS(profile=profiles[0], verify_cert=False, trust_env=True)
        for it in gis_dest.content.search("clone_item_test_*"):
            it.delete()
        for it in gis_dest.content.search("test_point_cloning*"):
            it.delete()
        res = gis_dest.content.clone_items([saved_map], preserve_item_id=True)
        assert not saved_map_item_id in [i.itemid for i in res]
        assert not source_item_id in [i.itemid for i in res]
        assert len(res) > 0
        for i in res:
            i.delete()
        pitem.delete()
    ...
    def test_agol_to_portal_preserve_item_id(self):
        """tests cloning from AGOL to Portal with preserve_item_id=True"""
        gis_objs = {
            "agol": GIS(profile=profiles[0], verify_cert=False, trust_env=True),
        }
        ...
        gis_dest = GIS(profile=profiles[1], verify_cert=False, trust_env=True)
        for it in gis_dest.content.search("clone_item_test_*"):
            it.delete()
        for it in gis_dest.content.search("test_point_cloning*"):
            it.delete()
        res = gis_dest.content.clone_items([saved_map], preserve_item_id=True)
        assert len(res) > 0
        assert saved_map.id in [r.itemid for r in res]
        for i in res:
            i.delete()
        pitem.delete()
        item.delete()
    ...
    def ent_to_ent(self):
        ...
        for it in gis_dest.content.search("clone_item_test_*"):
            it.delete()
        for it in gis_dest.content.search(
            "test_point_cloning* and owner:" + gis_dest.users.me.username
        ):
            it.delete()
        res = gis_dest.content.clone_items([saved_map])
        assert len(res) > 0
    ...
    def ent_to_ent_vw(self):
        ...
        for i in range(3):
            pt_geom = Point(
                {
                    "x": uniform(random_ext["xmin"], random_ext["xmax"]),
                    "y": uniform(random_ext["ymin"], random_ext["ymax"]),
                    "spatialReference": random_ext["spatialReference"],
                }
            )
            pt_attr = deepcopy(
                {f["name"]: "" for f in pitem.layers[0].properties.fields}
            )
        ...
        flc_view = pitem_mgr.create_view(
            name="clone_vw_test_" + f"{int(time.time())}"[:10],
            spatial_reference=dict(pitem_mgr.properties.spatialReference),
            capabilities="Query,Create,Update,Delete,Editing",
        )
        flc_view.layers[0].manager.update_definition(
            {"viewDefinitionQuery": "objectid < 2)"}
        )
        gis_dest = GIS(profile=profiles[1], verify_cert=False)
        # delete any views owned by user who will run the clone
        for it in gis_dest.content.search(
            "clone_vw_test_* AND owner:" + gis_dest.users.me.username
        ):
            it.delete()
