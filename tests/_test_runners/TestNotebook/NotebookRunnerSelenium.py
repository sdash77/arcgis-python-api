...
from _test_runners.TestNotebook.JupyterClassicNotebookServer import (
    JupyterClassicNotebookServer,
)

from _test_runners.TestNotebook.NotebookRunnerResult import NotebookRunnerResult
...
class NotebookRunnerSelenium:
    def __init__(
        ...
        self.notebook_file_name_no_ext = os.path.splitext(notebook_file_path)[0]
        ...
        self.notebook_file_name_no_ext = os.path.splitext(
            os.path.basename(self.notebook_file_path)
        )[0]
    ...
    def _run_notebook(self):
        ...
        self.orig_nb_tmp_path = os.path.join(
            self.notebook_file_dir,
            f"{self.notebook_file_name_no_ext}-orig{uuid4()}.ipynb",
        )
        shutil.copy(self.notebook_file_path, self.orig_nb_tmp_path)
        ...
        output_ipynb_path = os.path.join(
            self.output_dir, self.notebook_file_name_no_ext + ".ipynb"
        )
        output_html_path = os.path.join(
            self.output_dir, self.notebook_file_name_no_ext + ".html"
        )
        ...
        shutil.copy(self.orig_nb_tmp_path, self.notebook_file_path)
        os.remove(self.orig_nb_tmp_path)

        return NotebookRunnerResult(
            output_ipynb_path=output_ipynb_path, output_html_path=output_html_path
        )

    def _initialize_selenium(self):
        self.driver.implicitly_wait(GENERIC_SLEEP_SEC)
        nb_url = (
            self.active_jupyter_backend.base_url
    ...
    def _initialize_run_button_element(self):
        self._run_button = self.driver.find_element_by_xpath('//button[@title="Run"]')
    ...
    def _run_each_cell_until_bottom(self):
        ...
        while True:
            ...
            if seconds_cell_running > self.cell_timeout_sec:
                raise Exception(
                    f"Cell is running for {seconds_cell_running} "
    ...
    def _any_cell_currently_running(self):
        cell_prompt_input_elements = self.driver.find_elements_by_class_name(
            "input_prompt"
        )
    ...
    def _convert_ipynb_to_html(self, ipynb_path, html_path):
        with open(ipynb_path, "r", encoding="utf-8") as input_nb_file:
            # Execute input notebook
            input_nb = nbformat.read(
                input_nb_file, as_version=nbformat.current_nbformat
            )
            html_exporter = HTMLExporter()
            (body, resources) = html_exporter.from_notebook_node(input_nb)
            with open(html_path, "w", encoding="utf-8") as output_html:
