...
def run_notebook_tests(
    config, paths, output_dir, jenkins_job_url, notebook_runner="nbconvert"
):
    ...
    output_xml_path = os.path.join(
        output_dir, f"{notebook_runner}_notebook_tests_output.xml"
    )
    ...
    _write_index_html_file_for_outputted_notebooks(output_executed_notebooks_dir)
